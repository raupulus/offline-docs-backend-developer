...
source_url: https://www.sqlite.org/expridx.html
source_path: expridx.html
technology: sqlite
version: 3.53.4
license: Blessing
...
Normally, an SQL index references columns of a table. But an index can also be formed on expressions involving table columns, either directly by writing an expression instead of a bare column name in the CREATE INDEX statement, or by having the CREATE INDEX reference a [VIRTUAL generated column](gencol.md#vgc*).
...
Use a [CREATE INDEX](lang_createindex.md) statement to create a new index on one or more expressions just like you would to create an index on columns. The only difference is that expressions are listed as the elements to be indexed rather than column names.
...
2.  Expressions in CREATE INDEX statements may contain function calls, but only to functions whose output is always determined completely by its input parameters (a.k.a.: [deterministic functions](deterministic.md)). Obviously, functions like [random()](lang_corefunc.md#random) will not work well in an index. But also functions like [sqlite_version()](lang_corefunc.md#sqlite_version), though they are constant across any one database connection, are not constant across the life of the underlying database file, and hence may not be used in a CREATE INDEX statement. Note that [application-defined SQL functions](appfunc.md) are by default considered non-deterministic and may not be used in a CREATE INDEX statement unless the [SQLITE_DETERMINISTIC](c3ref/c_deterministic.md#sqlitedeterministic) flag is used when the function is registered.

    See also: [stale expression indexes](staleexpridx.md)

3.  Expressions in CREATE INDEX statements may not use subqueries.

4.  Expressions may only be used in CREATE INDEX statements, not within [UNIQUE](lang_createtable.md#uniqueconst) or [PRIMARY KEY](lang_createtable.md#primkeyconst) constraints within the [CREATE TABLE](lang_createtable.md) statement.

# 3. Compatibility

The ability to index expressions was added to SQLite with [version 3.9.0](releaselog/3_9_0.md) (2015-10-14). A database that uses an index on expressions will not be usable by earlier versions of SQLite.
