...
source_url: https://www.sqlite.org/affcase1.html
source_path: affcase1.html
technology: sqlite
version: 3.53.4
license: Blessing
...
For this first experiment, nothing else about the file format is changed. The OpenDocument is still a pile-of-files, only now each file is a row in an SQLite database rather than an entry in a ZIP archive. This simple change does not use the power of a relational database. Even so, this simple change shows some improvements. <span id="smaller"></span>
...
- Store an [automated undo/redo stack](undoredo.md) in a database table so that Undo could go back into prior edit sessions.
- Add [full text search](fts3.md#fts4) capabilities to the slide deck, or across multiple slide decks.
...
2.  The [atomic update capabilities](atomiccommit.md) of SQLite allow small incremental changes to be safely written into the document. This reduces total disk I/O and improves File/Save performance, enhancing the user experience.
...
These are just a few of the benefits of using SQLite as an application file format — the benefits that seem most likely to improve the user experience for applications like OpenOffice. Other applications might benefit from SQLite in different ways. See the [Application File Format](appfileformat.md) document for additional ideas.
