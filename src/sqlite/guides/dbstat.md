---
title: The DBSTAT Virtual Table
source_url: https://www.sqlite.org/dbstat.html
source_path: dbstat.html
technology: sqlite
version: 3.53.4
license: Blessing
...
The DBSTAT virtual table is a read-only [eponymous virtual table](vtab.md#epovtab) that returns information about the amount of disk space used to store the content of an SQLite database. Example use cases for the DBSTAT virtual table include the [sqlite3_analyzer.exe](sqlanalyze.md) utility program and the [table size pie-chart](https://sqlite.org/src/repo-tabsize) in the [Fossil-implemented](https://www.fossil-scm.org/) version control system for SQLite.

The DBSTAT virtual table is available on all [database connections](c3ref/sqlite3.md) when SQLite is built using the [SQLITE_ENABLE_DBSTAT_VTAB](compile.md#enable_dbstat_vtab) compile-time option.

The DBSTAT virtual table is an [eponymous virtual table](vtab.md#epovtab), meaning that is not necessary to run [CREATE VIRTUAL TABLE](lang_createvtab.md) to create an instance of the dbstat virtual table before using it. The "dbstat" module name can be used as if it were a table name to query the dbstat virtual table directly. For example:
...
<span id="dbstatagg"></span>

# 3. Aggregated Data

Beginning with SQLite version 3.31.0 (2020-01-22), the DBSTAT table has a new [hidden column](vtab.md#hiddencol) named "aggregate", which if constrained to be TRUE will cause DBSTAT to generate one row per btree in the database, rather than one row per page. When running in aggregated mode, the "path", "pagetype", and "pgoffset" columns are always NULL and the "pageno" column holds the number of pages in the entire btree, rather than the number of the page that corresponds to the row.

The following table shows the meanings of the (non-hidden) columns of DBSTAT in both normal and aggregated mode:

> <table data-border="1" data-cellpadding="5" data-cellspacing="0">
> <thead>
> <tr>
> <th>Column</th>
