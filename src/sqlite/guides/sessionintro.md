...
source_url: https://www.sqlite.org/sessionintro.html
source_path: sessionintro.html
technology: sqlite
version: 3.53.4
license: Blessing
...
The session extension provides a convenient mechanism to record changes to tables in an SQLite database, package those changes into a "changeset" or "patchset" file, and later apply the same set of changes to another database with the same schema and compatible starting data. A "changeset" may also be inverted and used to "undo" a session.

This document is an introduction to the session extension. The details of the interface are in the separate [Session Extension C-language Interface](session/intro.md) document.

## 1.1. Typical Use Case

Suppose SQLite is used as the [application file format](appfileformat.md) for a particular design application. Two users, Alice and Bob, each start with a baseline design that is about a gigabyte in size. They work all day, in parallel, each making their own customizations and tweaks to the design. At the end of the day, they would like to merge their changes together into a single unified design.
...
In other words, the session extension provides a facility for SQLite database files that is similar to the unix [patch](https://en.wikipedia.org/wiki/Patch_(Unix)) utility program, or to the "merge" capabilities of version control systems such as [Fossil](https://www.fossil-scm.org/), [Git](https://git-scm.com), or [Mercurial](http://www.mercurial-scm.org/).

## 1.2. Obtaining the Session Extension

Since [version 3.13.0](releaselog/3_13_0.md) (2016-05-18), the session extension has been included in the SQLite [amalgamation](amalgamation.md) source distribution. By default, the session extension is disabled. To enable it, build with the following compiler switches:

-DSQLITE_ENABLE_SESSION -DSQLITE_ENABLE_PREUPDATE_HOOK\

Or, if using the canonical build system, pass the --enable-session option to the configure script.

## 1.3. Limitations

- Prior to SQLite version 3.17.0, the session extension only worked with [rowid tables](rowidtable.md), not [WITHOUT ROWID](withoutrowid.md) tables. As of 3.17.0, both rowid and WITHOUT ROWID tables are supported. However, extra steps are needed to record primary keys for WITHOUT ROWID table changes.

- There is no support for [virtual tables](vtab.md). Changes to virtual tables are not captured.
...
- SQLite allows [NULL values](nulls.md) to be stored in PRIMARY KEY columns. However, the session extension ignores all such rows. No changes affecting rows with one or more NULL values in PRIMARY KEY columns are recorded by the sessions module.
...
- The target database may contain a row with the specified PRIMARY KEY values, but the other fields may contain values that do not match those stored as part of the changeset. This type of conflict is not detected when using a patchset.
...
The other information required to create a changeset or patchset is read from the database file when [sqlite3session_changeset()](session/sqlite3session_changeset.md) or [sqlite3session_patchset()](session/sqlite3session_patchset.md) is called. Specifically,
...
This section provides examples that demonstrate how to use the session extension.
...
1.  A session object (type sqlite3_session\*) is created by making a call to the [sqlite3session_create()](session/sqlite3session_create.md) API function.
...
2.  The session object is configured with a set of tables to monitor changes on.
    ...
    - By explicitly specifying tables using one call to [sqlite3session_attach()](session/sqlite3session_attach.md) for each table, or
    - By specifying that all tables in the database should be monitored for changes using a call to [sqlite3session_attach()](session/sqlite3session_attach.md) with a NULL argument, or
...
3.  Changes are made to the database by executing SQL statements. The session object records these changes.

4.  A changeset blob is extracted from the session object using a call to [sqlite3session_changeset()](session/sqlite3session_changeset.md) (or, if using patchsets, a call to the [sqlite3session_patchset()](session/sqlite3session_patchset.md) function).

5.  The session object is deleted using a call to the [sqlite3session_delete()](session/sqlite3session_delete.md) API function.

    It is not necessary to delete a session object after extracting a changeset or patchset from it. It can be left attached to the database handle and will continue monitoring for changes on the configured tables as before. However, if [sqlite3session_changeset()](session/sqlite3session_changeset.md) or [sqlite3session_patchset()](session/sqlite3session_patchset.md) is called a second time on a session object, the changeset or patchset will contain *all* changes that have taken place on the connection since the session was created. In other words, a session object is not reset or zeroed by a call to sqlite3session_changeset() or sqlite3session_patchset().
...
Applying a changeset to a database is simpler than capturing a changeset. Usually, a single call to [sqlite3changeset_apply()](session/sqlite3changeset_apply.md), as depicted in the example code below, suffices.
...
1.  The [sqlite3changeset_start()](session/sqlite3changeset_start.md) API is called to create and initialize an iterator to iterate through the contents of a changeset. Initially, the iterator points to no element at all.

2.  The first call to [sqlite3changeset_next()](session/sqlite3changeset_next.md) on the iterator moves it to point to the first change in the changeset (or to EOF, if the changeset is completely empty). sqlite3changeset_next() returns SQLITE_ROW if it moves the iterator to point to a valid entry, SQLITE_DONE if it moves the iterator to EOF, or an SQLite error code if an error occurs.

3.  If the iterator points to a valid entry, the [sqlite3changeset_op()](session/sqlite3changeset_op.md) API may be used to determine the type of change (INSERT, UPDATE or DELETE) that the iterator points to. Additionally, the same API can be used to obtain the name of the table the change applies to and its expected number of columns and primary key columns.

4.  If the iterator points to a valid INSERT or UPDATE entry, the [sqlite3changeset_new()](session/sqlite3changeset_new.md) API may be used to obtain the new.\* values within the change payload.

5.  If the iterator points to a valid DELETE or UPDATE entry, the [sqlite3changeset_old()](session/sqlite3changeset_old.md) API may be used to obtain the old.\* values within the change payload.

6.  An iterator is deleted using a call to the [sqlite3changeset_finalize()](session/sqlite3changeset_finalize.md) API. If an error occurred while iterating, an SQLite error code is returned (even if the same error code has already been returned by sqlite3changeset_next()). Or, if no error has occurred, SQLITE_OK is returned.
...
- Two or more changeset/patchsets may be combined using the [sqlite3changeset_concat()](session/sqlite3changeset_concat.md) or [sqlite3_changegroup](session/changegroup.md) interfaces.

- A changeset may be "inverted" using the [sqlite3changeset_invert()](session/sqlite3changeset_invert.md) API function. An inverted changeset undoes the changes made by the original. If changeset C<sup>+</sup> is the inverse of changeset C, then applying C and then C<sup>+</sup> to a database should leave the database unchanged.
