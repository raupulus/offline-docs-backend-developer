---
title: Automatic Undo/Redo With SQLite
source_url: https://www.sqlite.org/undoredo.html
source_path: undoredo.html
technology: sqlite
version: 3.53.4
license: Blessing
...
This page demonstrates how to use triggers to implement undo/redo logic for an application that uses SQLite as its [application file format](appfileformat.md).
...
Note the use of the [quote() SQL function](lang_corefunc.md#quote) in these triggers. The quote() function converts its argument into a form that is appropriate for inclusion in an SQL statement. Numeric values come through unchanged. Single quotes are added before and after strings and any internal single quotes are escaped. BLOB values are rendered using SQL-standard hexadecimal BLOB notation. The use of the quote() function ensures that the SQL statements used to undo and redo are always safe from SQL injection.
...
The demonstration code assumes that the SQLite database is opened and used as a database object named "db".
