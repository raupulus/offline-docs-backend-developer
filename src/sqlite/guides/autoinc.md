...
source_url: https://www.sqlite.org/autoinc.html
source_path: autoinc.html
technology: sqlite
version: 3.53.4
license: Blessing
...
2.  In SQLite, a column with type INTEGER PRIMARY KEY is an alias for the [ROWID](lang_createtable.md#rowid) (except in [WITHOUT ROWID](withoutrowid.md) tables) which is always a 64-bit signed integer.

3.  On an [INSERT](lang_insert.md), if the ROWID or INTEGER PRIMARY KEY column is not explicitly given a value, then it will be filled automatically with an unused integer, usually one more than the largest ROWID currently in use. This is true regardless of whether or not the AUTOINCREMENT keyword is used.
...
In SQLite, table rows normally have a 64-bit signed integer [ROWID](lang_createtable.md#rowid) which is unique among all rows in the same table. ([WITHOUT ROWID](withoutrowid.md) tables are the exception.)

You can access the ROWID of an SQLite table using one of the special column names ROWID, \_ROWID\_, or OID. Except if you declare an ordinary table column to use one of those special names, then the use of that name will refer to the declared column not to the internal ROWID.

If a table contains a column of type [INTEGER PRIMARY KEY](lang_createtable.md#rowid), then that column becomes an alias for the ROWID. You can then access the ROWID using any of four different names, the original three names described above or the name given to the [INTEGER PRIMARY KEY](lang_createtable.md#rowid) column. All these names are aliases for one another and work equally well in any context.
...
If no ROWID is specified on the insert, or if the specified ROWID has a value of NULL, then an appropriate ROWID is created automatically. The usual algorithm is to give the newly created row a ROWID that is one larger than the largest ROWID in the table prior to the insert. If the table is initially empty, then a ROWID of 1 is used. If the largest ROWID is equal to the largest possible integer (9223372036854775807) then the database engine starts picking positive candidate ROWIDs at random until it finds one that is not previously used. If no unused ROWID can be found after a reasonable number of attempts, the insert operation fails with an [SQLITE_FULL](rescode.md#full) error. If no negative ROWID values are inserted explicitly, then automatically generated ROWID values will always be greater than zero.
