---
title: Profiling SQL Queries
source_url: https://www.sqlite.org/profile.html
source_path: profile.html
technology: sqlite
version: 3.53.4
license: Blessing
...
SQLite contains built-in support for profiling SQL queries, but it is not enabled by default. In order to enable support for query profiling, SQLite must be compiled with the [following option](compile.md#enable_stmt_scanstatus):

-DSQLITE_ENABLE_STMT_SCANSTATUS\

Building SQLite with this option enables the [sqlite3_stmt_scanstatus_v2()](c3ref/stmt_scanstatus.md) API, which provides access to the various profiling metrics. The remainder of this page discusses the profiling reports generated by the SQLite [command-line shell](cli.md) using these metrics, rather than the API directly.

The profiling reports generated by the shell are very similar to the query plan reports generated by the [EXPLAIN QUERY PLAN](eqp.md) command. This page assumes that the reader is familiar with this format.
...
The most complicated part of the example above is understanding the query plan - the portion of the report that would also be generated by an [EXPLAIN QUERY PLAN](eqp.md) command.

The following query uses an [automatic index](optoverview.md#autoindex) and an external sort:
...
The output below is from query 15c of the [Join Order Benchmark](https://github.com/datalevin/datalevin/tree/master/benchmarks/JOB-bench).
