...
source_url: https://www.sqlite.org/assert.html
source_path: assert.html
technology: sqlite
version: 3.53.4
license: Blessing
...
The assert(X) macro is [part of standard C](https://en.wikipedia.org/wiki/Assert.h), in the \<assert.h\> header file. SQLite adds three other assert()-like macros named NEVER(X), ALWAYS(X), and testcase(X).
...
All tests must give the same answer in all three builds. See the ["How SQLite Is Tested"](testing.md) document for more detail.
