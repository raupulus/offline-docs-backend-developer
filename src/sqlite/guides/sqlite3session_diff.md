---
title: Load The Difference Between Tables Into A Session
source_url: https://www.sqlite.org/session/sqlite3session_diff.html
source_path: session/sqlite3session_diff.html
technology: sqlite
version: 3.53.4
license: Blessing
...
If it is not already attached to the session object passed as the first argument, this function attaches table zTbl in the same manner as the [sqlite3session_attach()](../session/sqlite3session_attach.md) function. If zTbl does not exist, or if it does not have a primary key, this function is a no-op (but does not return an error).
...
To clarify, if this function is called and then a changeset constructed using [sqlite3session_changeset()](../session/sqlite3session_changeset.md), then after applying that changeset to database zFrom the contents of the two compatible tables would be identical.
