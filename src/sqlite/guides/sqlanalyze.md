---
title: The sqlite3_analyzer.exe Utility Program
source_url: https://www.sqlite.org/sqlanalyze.html
source_path: sqlanalyze.html
technology: sqlite
version: 3.53.4
license: Blessing
...
The output is a human-readable ASCII text report that provides information on the space utilization of the database file. The report is intended to be self-explanatory, though there is some [additional explanation](sqlanalyze.md#defs) of the various parameters reported toward the end of the report.

The output is also valid SQL. Most of the report text is contained within a header comment, with various SQL statements that create and initialize a database at the [end of the report](sqlanalyze.md#sqlx). The constructed database contains the raw data from which the report was extracted. Hence the original report can be read into an instance of the [command-line shell](cli.md) and then the raw data can be queried to dig deeper into the space utilization of a particular database file.

## 1.1. Implementation

The `sqlite3_analyzer.exe` program is a [TCL](http://www.tcl.tk/) program that uses the [dbstat virtual table](dbstat.md) to gather information about the database file and then format that information neatly.
