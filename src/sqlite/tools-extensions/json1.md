...
source_url: https://www.sqlite.org/json1.html
source_path: json1.html
technology: sqlite
version: 3.53.4
license: Blessing
...
By default, SQLite supports thirty functions and two operators for dealing with JSON values. There are also four [table-valued functions](vtab.md#tabfunc2) that can be used to decompose a JSON string. All of the functions listed below have the [SQLITE_INNOCUOUS](c3ref/c_deterministic.md#sqliteinnocuous) and [SQLITE_DETERMINISTIC](c3ref/c_deterministic.md#sqlitedeterministic) flags.
...
5.  [jsonb_array_insert](#jarrayins)(*json*,*path*,*value*,...)
6.  [json_array_insert](#jarrayins)(*json*,*path*,*value*,...)
7.  [json_array_length](#jarraylen)(*json*)\
    [json_array_length](#jarraylen)(*json*,*path*)
8.  [json_error_position](#jerr)(*json*)
9.  [json_extract](#jex)(*json*,*path*,...)
10. [jsonb_extract](#jexb)(*json*,*path*,...)
11. *json* [-\>](#jptr) *path*
12. *json* [-\>\>](#jptr) *path*
13. [json_insert](#jins)(*json*,*path*,*value*,...)
14. [jsonb_insert](#jinsb)(*json*,*path*,*value*,...)
15. [json_object](#jobj)(*label1*,*value1*,...)
16. [jsonb_object](#jobjb)(*label1*,*value1*,...)
17. [json_patch](#jpatch)(*json*1,json2)
18. [jsonb_patch](#jpatchb)(*json*1,json2)
19. [json_pretty](#jpretty)(*json*)
20. [json_quote](#jquote)(*value*)
21. [json_remove](#jrm)(*json*,*path*,...)
22. [jsonb_remove](#jrmb)(*json*,*path*,...)
23. [json_replace](#jrepl)(*json*,*path*,*value*,...)
24. [jsonb_replace](#jreplb)(*json*,*path*,*value*,...)
25. [json_set](#jset)(*json*,*path*,*value*,...)
26. [jsonb_set](#jsetb)(*json*,*path*,*value*,...)
27. [json_type](#jtype)(*json*)\
    [json_type](#jtype)(*json*,*path*)
28. [json_valid](#jvalid)(*json*)\
    [json_valid](#jvalid)(*json*,flags)

There are four [aggregate SQL functions](lang_aggfunc.md):

1.  [json_group_array](#jgrouparray)(*value*)
2.  [jsonb_group_array](#jgrouparrayb)(*value*)
3.  [json_group_object](#jgroupobject)(*label*,*value*)
4.  [jsonb_group_object](#jgroupobjectb)(*label*,*value*)

The four [table-valued functions](vtab.md#tabfunc2) are:

1.  [json_each](#jeach)(*json*)\
    [json_each](#jeach)(*json*,*path*)
2.  [json_tree](#jtree)(*json*)\
    [json_tree](#jtree)(*json*,*path*)
3.  [jsonb_each](#jeach)(*json*)\
    [jsonb_each](#jeach)(*json*,*path*)
4.  [jsonb_tree](#jtree)(*json*)\
    [jsonb_tree](#jtree)(*json*,*path*)
...
The way in which SQLite handles invalid JSONB might change from one version of SQLite to the next. The system follows the garbage-in/garbage-out rule: If you feed the JSON functions invalid JSONB, you get back an invalid answer. If you are in doubt about the validity of our JSONB, use the [json_valid()](json1.md#jvalid) function to verify it.

We do make this one promise: Malformed JSONB will never cause a memory error or similar problem that might lead to a vulnerability. Invalid JSONB might lead to crazy answers, or it might cause queries to abort, but it won't cause a crash. <span id="jsonpath"></span>
...
- <span class="jex">json_set('\[0,1,2\]','\$\[#\]','new')</span> <span class="jans">→ '\[0,1,2,"new"\]'</span>
...
However, if a *value* argument comes directly from the result of another JSON function or from [the -\> operator](json1.md#jptr) (but not [the -\>\> operator](json1.md#jptr)), then the argument is understood to be actual JSON and the complete JSON is inserted rather than a quoted string.
...
- <span class="jex">json_object('ex','\[52,3.14159\]')</span> <span class="jans">→ '{"ex":"\[52,3.14159\]"}'</span>
- <span class="jex">json_object('ex',('\[52,3.14159\]'-\>\>'\$'))</span> <span class="jans">→ '{"ex":"\[52,3.14159\]"}'</span>

But if the *value* argument in the outer json_object() call is the result of another JSON function like [json()](json1.md#jmini) or [json_array()](json1.md#jarray), then the value is understood to be actual JSON and is inserted as such:

- <span class="jex">json_object('ex',json('\[52,3.14159\]'))</span> <span class="jans">→ '{"ex":\[52,3.14159\]}'</span>
- <span class="jex">json_object('ex',json_array(52,3.14159))</span> <span class="jans">→ '{"ex":\[52,3.14159\]}'</span>
- <span class="jex">json_object('ex','\[52,3.14159\]'-\>'\$')</span> <span class="jans">→ '{"ex":\[52,3.14159\]}'</span>

To be clear: "*json*" arguments are always interpreted as JSON regardless of where the value for that argument comes from. But "*value*" arguments are only interpreted as JSON if those arguments come directly from another JSON function or [the -\> operator](json1.md#jptr).
...
The current implementation of this JSON library uses a recursive descent parser. In order to avoid using excess stack space, any JSON input that has more than 1000 levels of nesting is considered invalid. Limits on nesting depth are allowed for compatible implementations of JSON by [RFC-8259 section 9](https://tools.ietf.org/html/rfc8259#section-9). <span id="json5"></span>
...
To convert string X from JSON5 into canonical JSON, invoke "[json(X)](json1.md#jmini)". The output of the "[json()](json1.md#jmini)" function will be canonical JSON regardless of any JSON5 extensions that are present in the input. For backwards compatibility, the [json_valid(X)](json1.md#jvalid) function without a "flags" argument continues to report false for inputs that are not canonical JSON, even if the input is JSON5 that the function is able to understand. To determine whether or not an input string is valid JSON5, include the 0x02 bit in the "flags" argument to json_valid: "`json_valid(X,2)`".
...
The [aggregate JSON SQL functions](json1.md#jgroupobjectb) are an exception to this rule. Those functions all do their processing using text instead of JSONB. So for the aggregate JSON SQL functions, it is more efficient for the arguments to be supplied using "`json_`" functions than "`jsonb_`" functions.

- `... json_group_array(json(A))) ...`   ← More efficient.
- `... json_group_array(jsonb(A))) ...`   ← Less efficient.

<span id="jblobbug"></span>
...
SQLite does not strictly enforce JSON string syntax in PATHs. If you use [json_set()](json1.md#jset) or similar to add new elements to an object, and the PATH argument contains backslash escapes that are not valid in JSON, SQLite does not try to prevent you from doing so and you can end up with invalid JSON.

# 4. Function Details

The following sections provide additional detail on the operation of the various JSON functions and operators: <span id="jmini"></span>
...
- <span class="jex">json(' { "this" : "is", "a": \[ "test" \] } ')</span> <span class="jans">→ '{"this":"is","a":\["test"\]}'</span>
...
If X is a BLOB and appears to be JSONB, then this routine simply returns a copy of X. Only the outer-most element of the JSONB input is examined, however. The deep structure of the JSONB is not validated. <span id="jarray"></span>
...
An argument with SQL type TEXT is normally converted into a quoted JSON string. However, if the argument is the output from another json1 function, then it is stored as JSON. This allows calls to json_array() and [json_object()](json1.md#jobj) to be nested. The [json()](json1.md#jmini) function can also be used to force strings to be recognized as JSON.

Examples:

- <span class="jex">json_array(1,2,'3',4)</span> <span class="jans">→ '\[1,2,"3",4\]'</span>
- <span class="jex">json_array('\[1,2\]')</span> <span class="jans">→ '\["\[1,2\]"\]'</span>
- <span class="jex">json_array(json_array(1,2))</span> <span class="jans">→ '\[\[1,2\]\]'</span>
- <span class="jex">json_array(1,null,'3','\[4,5\]','{"six":7.7}')</span> <span class="jans">→ '\[1,null,"3","\[4,5\]","{\\six\\:7.7}"\]'</span>
- <span class="jex">json_array(1,null,'3',json('\[4,5\]'),json('{"six":7.7}'))</span> <span class="jans">→ '\[1,null,"3",\[4,5\],{"six":7.7}\]'</span>

<span id="jarrayb"></span>

## 4.4. The jsonb_array() function

The jsonb_array() SQL function works just like the [json_array()](json1.md#jarray) function except that it returns the constructed JSON array in the SQLite's private JSONB format rather than in the standard RFC 8259 text format. <span id="jarrayins"></span>

## 4.5. The json_array_insert() and jsonb_array_insert() functions

The json_array_insert() and jsonb_array_insert() SQL functions operate similarly to the [json_replace()](json1.md#jrepl) and [jsonb_replace](json1.md#jreplb) functions, with just two differences:
...
As with json_replace() and jsonb_replace(), the PATH and VALUE arguments can be repeated multiple times. The modified JSON is returned, either in a text format for json_array_insert() or in the binary BLOB format for jsonb_array_insert().

Examples:

- <span class="jex">json_array_insert('\[1,2,3\]','\$\[1\]','new')</span> <span class="jans">→ \[1,"new",2,3\]</span>
- <span class="jex">json_array_insert('{"a":\[1,2,3\]}', '\$.a\[0\]','new')</span> <span class="jans">→ {"a":\["new",1,2,3\]}</span>

<span id="jarraylen"></span>

## 4.6. The json_array_length() function
...
- <span class="jex">json_array_length('\[1,2,3,4\]')</span> <span class="jans">→ 4</span>
- <span class="jex">json_array_length('\[1,2,3,4\]', '\$')</span> <span class="jans">→ 4</span>
- <span class="jex">json_array_length('\[1,2,3,4\]', '\$\[2\]')</span> <span class="jans">→ 0</span>
- <span class="jex">json_array_length('{"one":\[1,2,3\]}')</span> <span class="jans">→ 0</span>
- <span class="jex">json_array_length('{"one":\[1,2,3\]}', '\$.one')</span> <span class="jans">→ 3</span>
- <span class="jex">json_array_length('{"one":\[1,2,3\]}', '\$.two')</span> <span class="jans">→ NULL</span>

<span id="jerr"></span>

## 4.7. The json_error_position() function
...
The json_error_position() function was added with SQLite version 3.42.0 (2023-05-16). <span id="jex"></span>

## 4.8. The json_extract() function
...
- <span class="jex">json_extract('{"a":2,"c":\[4,5,{"f":7}\]}', '\$')</span> <span class="jans">→ '{"a":2,"c":\[4,5,{"f":7}\]}'</span>
- <span class="jex">json_extract('{"a":2,"c":\[4,5,{"f":7}\]}', '\$.c')</span> <span class="jans">→ '\[4,5,{"f":7}\]'</span>
- <span class="jex">json_extract('{"a":2,"c":\[4,5,{"f":7}\]}', '\$.c\[2\]')</span> <span class="jans">→ '{"f":7}'</span>
- <span class="jex">json_extract('{"a":2,"c":\[4,5,{"f":7}\]}', '\$.c\[2\].f')</span> <span class="jans">→ 7</span>
- <span class="jex">json_extract('{"a":2,"c":\[4,5\],"f":7}','\$.c','\$.a')</span> <span class="jans">→ '\[\[4,5\],2\]'</span>
- <span class="jex">json_extract('{"a":2,"c":\[4,5\],"f":7}','\$.c\[#-1\]')</span> <span class="jans">→ 5</span>
- <span class="jex">json_extract('{"a":2,"c":\[4,5,{"f":7}\]}', '\$.x')</span> <span class="jans">→ NULL</span>
- <span class="jex">json_extract('{"a":2,"c":\[4,5,{"f":7}\]}', '\$.x', '\$.a')</span> <span class="jans">→ '\[null,2\]'</span>
- <span class="jex">json_extract('{"a":"xyz"}', '\$.a')</span> <span class="jans">→ 'xyz'</span>
- <span class="jex">json_extract('{"a":null}', '\$.a')</span> <span class="jans">→ NULL</span>
...
## 4.9. The jsonb_extract() function

The jsonb_extract() function works the same as the [json_extract()](json1.md#jex) function, except in cases where json_extract() would normally return a text JSON array object, this routine returns the array or object in the JSONB format. For the common case where a text, numeric, null, or boolean JSON element is returned, this routine works exactly the same as json_extract(). <span id="jptr"></span>
...
Both the -\> and -\>\> operators select the same subcomponent of the JSON to their left. The difference is that -\> always returns a JSON representation of that subcomponent and the -\>\> operator always returns an SQL representation of that subcomponent. Thus, these operators are subtly different from a two-argument [json_extract()](json1.md#jex) function call. A call to json_extract() with two arguments will return a JSON representation of the subcomponent if and only if the subcomponent is a JSON array or object, and will return an SQL representation of the subcomponent if the subcomponent is a JSON null, string, or numeric value.

When the -\> operator returns JSON, it always returns the RFC 8565 text representation of that JSON, not JSONB. Use the [jsonb_extract()](json1.md#jexb) function if you need a subcomponent in the JSONB format.
...
- <span class="jex">'{"a":2,"c":\[4,5,{"f":7}\]}' -\> '\$'</span> <span class="jans">→ '{"a":2,"c":\[4,5,{"f":7}\]}'</span>
- <span class="jex">'{"a":2,"c":\[4,5,{"f":7}\]}' -\> '\$.c'</span> <span class="jans">→ '\[4,5,{"f":7}\]'</span>
- <span class="jex">'{"a":2,"c":\[4,5,{"f":7}\]}' -\> 'c'</span> <span class="jans">→ '\[4,5,{"f":7}\]'</span>
- <span class="jex">'{"a":2,"c":\[4,5,{"f":7}\]}' -\> '\$.c\[2\]'</span> <span class="jans">→ '{"f":7}'</span>
- <span class="jex">'{"a":2,"c":\[4,5,{"f":7}\]}' -\> '\$.c\[2\].f'</span> <span class="jans">→ '7'</span>
- <span class="jex">'{"a":2,"c":\[4,5,{"f":7}\]}' -\>\> '\$.c\[2\].f'</span> <span class="jans">→ 7</span>
- <span class="jex">'{"a":2,"c":\[4,5,{"f":7}\]}' -\> 'c' -\> 2 -\>\> 'f'</span> <span class="jans">→ 7</span>
- <span class="jex">'{"a":2,"c":\[4,5\],"f":7}' -\> '\$.c\[#-1\]'</span> <span class="jans">→ '5'</span>
- <span class="jex">'{"a":2,"c":\[4,5,{"f":7}\]}' -\> '\$.x'</span> <span class="jans">→ NULL</span>
- <span class="jex">'\[11,22,33,44\]' -\> 3</span> <span class="jans">→ '44'</span>
- <span class="jex">'\[11,22,33,44\]' -\>\> 3</span> <span class="jans">→ 44</span>
- <span class="jex">'{"a":"xyz"}' -\> '\$.a'</span> <span class="jans">→ '"xyz"'</span>
- <span class="jex">'{"a":"xyz"}' -\>\> '\$.a'</span> <span class="jans">→ 'xyz'</span>
- <span class="jex">'{"a":null}' -\> '\$.a'</span> <span class="jans">→ 'null'</span>
- <span class="jex">'{"a":null}' -\>\> '\$.a'</span> <span class="jans">→ NULL</span>

<span id="jins"></span> <span id="jrepl"></span> <span id="jset"></span>

## 4.11. The json_insert(), json_replace, and json_set() functions
...
If the value of a path/value pair is an SQLite TEXT value, then it is normally inserted as a quoted JSON string, even if the string looks like valid JSON. However, if the value is the result of another json function (such as [json()](json1.md#jmini) or [json_array()](json1.md#jarray) or [json_object()](json1.md#jobj)) or if it is the result of [the -\> operator](json1.md#jptr), then it is interpreted as JSON and is inserted as JSON retaining all of its substructure. Values that are the result of [the -\>\> operator](json1.md#jptr) are always interpreted as TEXT and are inserted as a JSON string even if they look like valid JSON.
...
- <span class="jex">json_insert('\[1,2,3,4\]','\$\[#\]',99)</span> <span class="jans">→ '\[1,2,3,4,99\]'</span>
- <span class="jex">json_insert('\[1,\[2,3\],4\]','\$\[1\]\[#\]',99)</span> <span class="jans">→ '\[1,\[2,3,99\],4\]'</span>

Use the [json_array_insert()](json1.md#jarrayins) function to insert elements at the beginning or in the middle of an array.

Other examples:

- <span class="jex">json_insert('{"a":2,"c":4}', '\$.a', 99)</span> <span class="jans">→ '{"a":2,"c":4}'</span>
- <span class="jex">json_insert('{"a":2,"c":4}', '\$.e', 99)</span> <span class="jans">→ '{"a":2,"c":4,"e":99}'</span>
- <span class="jex">json_replace('{"a":2,"c":4}', '\$.a', 99)</span> <span class="jans">→ '{"a":99,"c":4}'</span>
- <span class="jex">json_replace('{"a":2,"c":4}', '\$.e', 99)</span> <span class="jans">→ '{"a":2,"c":4}'</span>
- <span class="jex">json_set('{"a":2,"c":4}', '\$.a', 99)</span> <span class="jans">→ '{"a":99,"c":4}'</span>
- <span class="jex">json_set('{"a":2,"c":4}', '\$.e', 99)</span> <span class="jans">→ '{"a":2,"c":4,"e":99}'</span>
- <span class="jex">json_set('{"a":2,"c":4}', '\$.c', '\[97,96\]')</span> <span class="jans">→ '{"a":2,"c":"\[97,96\]"}'</span>
- <span class="jex">json_set('{"a":2,"c":4}', '\$.c', json('\[97,96\]'))</span> <span class="jans">→ '{"a":2,"c":\[97,96\]}'</span>
- <span class="jex">json_set('{"a":2,"c":4}', '\$.c', json_array(97,96))</span> <span class="jans">→ '{"a":2,"c":\[97,96\]}'</span>

<span id="jinsb"></span> <span id="jreplb"></span> <span id="jsetb"></span>

## 4.12. The jsonb_insert(), jsonb_replace, and jsonb_set() functions

The jsonb_insert(), jsonb_replace(), and jsonb_set() functions work the same as [json_insert()](json1.md#jins), [json_replace()](json1.md#jrepl), and [json_set()](json1.md#jset), respectively, except that "`jsonb_`" versions return their result in the binary JSONB format. <span id="jobj"></span>

## 4.13. The json_object() function
...
An argument with SQL type TEXT is normally converted into a quoted JSON string even if the input text is well-formed JSON. However, if the argument is the direct result from another JSON function or [the -\> operator](json1.md#jptr) (but not [the -\>\> operator](json1.md#jptr)), then it is treated as JSON and all of its JSON type information and substructure is preserved. This allows calls to json_object() and [json_array()](json1.md#jarray) to be nested. The [json()](json1.md#jmini) function can also be used to force strings to be recognized as JSON.

Examples:

- <span class="jex">json_object('a',2,'c',4)</span> <span class="jans">→ '{"a":2,"c":4}'</span>
- <span class="jex">json_object('a',2,'c','{e:5}')</span> <span class="jans">→ '{"a":2,"c":"{e:5}"}'</span>
- <span class="jex">json_object('a',2,'c',json_object('e',5))</span> <span class="jans">→ '{"a":2,"c":{"e":5}}'</span>

<span id="jobjb"></span>

## 4.14. The jsonb_object() function

The jsonb_object() function works just like the [json_object()](json1.md#jobj) function except that the generated object is returned in the binary JSONB format. <span id="jpatch"></span>

## 4.15. The json_patch() function

The json_patch(T,P) SQL function runs the [RFC-7396](https://tools.ietf.org/html/rfc7396) MergePatch algorithm to apply patch P against input T. The patched copy of T is returned.

MergePatch can add, modify, or delete elements of a JSON Object, and so for JSON Objects, the json_patch() routine is a generalized replacement for [json_set()](json1.md#jset) and [json_remove()](json1.md#jrm). However, MergePatch treats JSON Array objects as atomic. MergePatch cannot append to an Array nor modify individual elements of an Array. It can only insert, replace, or delete the whole Array as a single unit. Hence, json_patch() is not as useful when dealing with JSON that includes Arrays, especially Arrays with lots of substructure.

Examples:

- <span class="jex">json_patch('{"a":1,"b":2}','{"c":3,"d":4}')</span> <span class="jans">→ '{"a":1,"b":2,"c":3,"d":4}'</span>
- <span class="jex">json_patch('{"a":\[1,2\],"b":2}','{"a":9}')</span> <span class="jans">→ '{"a":9,"b":2}'</span>
- <span class="jex">json_patch('{"a":\[1,2\],"b":2}','{"a":null}')</span> <span class="jans">→ '{"b":2}'</span>
- <span class="jex">json_patch('{"a":1,"b":2}','{"a":9,"b":null,"c":8}')</span> <span class="jans">→ '{"a":9,"c":8}'</span>
- <span class="jex">json_patch('{"a":{"x":1,"y":2},"b":3}','{"a":{"y":9},"c":8}')</span> <span class="jans">→ '{"a":{"x":1,"y":9},"b":3,"c":8}'</span>

<span id="jpatchb"></span>

## 4.16. The jsonb_patch() function

The jsonb_patch() function works just like the [json_patch()](json1.md#jpatch) function except that the patched JSON is returned in the binary JSONB format. <span id="jpretty"></span>

## 4.17. The json_pretty() function

The json_pretty() function works like [json()](json1.md#jmini) except that it adds extra whitespace to make the JSON result easier for humans to read. The first argument is the JSON or JSONB that is to be pretty-printed. The optional second argument is a text string that is used for indentation. If the second argument is omitted or is NULL, then indentation is four spaces per level.

The json_pretty() function was added with SQLite version 3.46.0 (2024-05-23). <span id="jrm"></span>

## 4.18. The json_remove() function
...
- <span class="jex">json_remove('\[0,1,2,3,4\]','\$\[2\]')</span> <span class="jans">→ '\[0,1,3,4\]'</span>
- <span class="jex">json_remove('\[0,1,2,3,4\]','\$\[2\]','\$\[0\]')</span> <span class="jans">→ '\[1,3,4\]'</span>
- <span class="jex">json_remove('\[0,1,2,3,4\]','\$\[0\]','\$\[2\]')</span> <span class="jans">→ '\[1,2,4\]'</span>
- <span class="jex">json_remove('\[0,1,2,3,4\]','\$\[#-1\]','\$\[0\]')</span> <span class="jans">→ '\[1,2,3\]'</span>
- <span class="jex">json_remove('{"x":25,"y":42}')</span> <span class="jans">→ '{"x":25,"y":42}'</span>
- <span class="jex">json_remove('{"x":25,"y":42}','\$.z')</span> <span class="jans">→ '{"x":25,"y":42}'</span>
- <span class="jex">json_remove('{"x":25,"y":42}','\$.y')</span> <span class="jans">→ '{"x":25}'</span>
- <span class="jex">json_remove('{"x":25,"y":42}','\$')</span> <span class="jans">→ NULL</span>

<span id="jrmb"></span>

## 4.19. The jsonb_remove() function

The jsonb_remove() function works just like the [json_remove()](json1.md#jrm) function except that the edited JSON result is returned in the binary JSONB format. <span id="jtype"></span>

## 4.20. The json_type() function
...
- <span class="jex">json_type('{"a":\[2,3.5,true,false,null,"x"\]}')</span> <span class="jans">→ 'object'</span>
- <span class="jex">json_type('{"a":\[2,3.5,true,false,null,"x"\]}','\$')</span> <span class="jans">→ 'object'</span>
- <span class="jex">json_type('{"a":\[2,3.5,true,false,null,"x"\]}','\$.a')</span> <span class="jans">→ 'array'</span>
- <span class="jex">json_type('{"a":\[2,3.5,true,false,null,"x"\]}','\$.a\[0\]')</span> <span class="jans">→ 'integer'</span>
- <span class="jex">json_type('{"a":\[2,3.5,true,false,null,"x"\]}','\$.a\[1\]')</span> <span class="jans">→ 'real'</span>
- <span class="jex">json_type('{"a":\[2,3.5,true,false,null,"x"\]}','\$.a\[2\]')</span> <span class="jans">→ 'true'</span>
- <span class="jex">json_type('{"a":\[2,3.5,true,false,null,"x"\]}','\$.a\[3\]')</span> <span class="jans">→ 'false'</span>
- <span class="jex">json_type('{"a":\[2,3.5,true,false,null,"x"\]}','\$.a\[4\]')</span> <span class="jans">→ 'null'</span>
- <span class="jex">json_type('{"a":\[2,3.5,true,false,null,"x"\]}','\$.a\[5\]')</span> <span class="jans">→ 'text'</span>
- <span class="jex">json_type('{"a":\[2,3.5,true,false,null,"x"\]}','\$.a\[6\]')</span> <span class="jans">→ NULL</span>

<span id="jvalid"></span>

## 4.21. The json_valid() function
...
- **0x02** → The input is text that is JSON with [JSON5](json1.md#json5) extensions described above.
- **0x04** → The input is a BLOB that superficially appears to be [JSONB](json1.md#jsonbx).
- **0x08** → The input is a BLOB that strictly conforms to the internal [JSONB](json1.md#jsonbx) format.

By combining bits, the following useful values of Y can be derived:

- **1** → X is RFC-8259 JSON text
- **2** → X is [JSON5](json1.md#json5) text
- **4** → X is probably [JSONB](json1.md#jsonbx)
- **5** → X is RFC-8259 JSON text or [JSONB](json1.md#jsonbx)
- **6** → X is [JSON5](json1.md#json5) text or [JSONB](json1.md#jsonbx) ← *This is probably the value you want*
- **8** → X is strictly conforming [JSONB](json1.md#jsonbx)
- **9** → X is RFC-8259 or strictly conforming [JSONB](json1.md#jsonbx)
- **10** → X is JSON5 or strictly conforming [JSONB](json1.md#jsonbx)

The Y parameter is optional. If omitted, it defaults to 1, which means that the default behavior is to return true only if the input X is strictly conforming RFC-8259 JSON text without any extensions. This makes the one-argument version of json_valid() compatible with older versions of SQLite, prior to the addition of support for [JSON5](json1.md#json5) and [JSONB](json1.md#jsonbx).

The difference between 0x04 and 0x08 bits in the Y parameter is that 0x04 only examines the outer wrapper of the BLOB to see if it superficially looks like [JSONB](json1.md#jsonbx). This is sufficient for most purposes and is very fast. The 0x08 bit does a thorough examination of all internal details of the BLOB. The 0x08 bit takes time that is linear in the size of the X input and is much slower. The 0x04 bit is recommended for most purposes.
...
- <span class="jex">json_valid('{"x":35}')</span> <span class="jans">→ 1</span>
- <span class="jex">json_valid('{x:35}')</span> <span class="jans">→ 0</span>
- <span class="jex">json_valid('{x:35}',6)</span> <span class="jans">→ 1</span>
- <span class="jex">json_valid('{"x":35')</span> <span class="jans">→ 0</span>
- <span class="jex">json_valid(NULL)</span> <span class="jans">→ NULL</span>

<span id="jquote"></span>

## 4.22. The json_quote() function
...
- <span class="jex">json_quote(3.14159)</span> <span class="jans">→ '3.14159'</span>
- <span class="jex">json_quote('verdant')</span> <span class="jans">→ '"verdant"'</span>
- <span class="jex">json_quote(NULL)</span> <span class="jans">→ 'null'</span>
- <span class="jex">json_quote('\[1\]')</span> <span class="jans">→ '"\[1\]"'</span>
- <span class="jex">json_quote(json('\[1\]'))</span> <span class="jans">→ '\[1\]'</span>
- <span class="jex">json_quote('\[1,')</span> <span class="jans">→ '"\[1,"'</span>

<span id="jgrouparray"></span> <span id="jgroupobject"></span> <span id="jgrouparrayb"></span> <span id="jgroupobjectb"></span>

## 4.23. Array and object aggregate functions

The json_group_array(X) function is an [aggregate SQL function](lang_aggfunc.md) that returns a JSON array comprised of all X values in the aggregation. Similarly, the json_group_object(NAME,VALUE) function returns a JSON object comprised of all NAME/VALUE pairs in the aggregation. The "`jsonb_`" variants are the same except that they return their result in the binary [JSONB](json1.md#jsonbx) format.
...
The json_group_object() and jsonb_group_object() functions ignore input rows for which the NAME argument is NULL. The json_group_array() and jsonb_group_array() functions include all inputs in the resulting array, even NULL inputs. <span id="jeach"></span> <span id="jtree"></span> <span id="jbeach"></span> <span id="jbtree"></span>

## 4.24. Table valued functions for parsing JSON: json_each(), jsonb_each(), json_tree(), and jsonb_tree()

The json_each(X), jsonb_each(X), json_tree(X), and jsonb_tree(X) [table-valued functions](vtab.md#tabfunc2) all walk the JSON value provided as their first argument and return one row for each element. The json_each(X) and jsonb_each(X) functions walk only the immediate children of the top-level array or object, or just the top-level element itself if the top-level element is a primitive value. The json_tree(X) and jsonb_tree(X) functions recursively walk through the JSON substructure starting with the top-level element.
...
The jsonb_each() and jsonb_tree() variants are available beginning with SQLite version 3.51.0 (2025-11-04). These two variants work the same as their counterparts with out the "b" except that the "value" column returns [JSONB](json1.md#jsonbx) instead of text JSON when the "type" column is 'object' or 'array'.

The following chart summarizes the differences between the various JSON table-valued functions:

<table data-border="1" data-cellpadding="5" data-cellspacing="0" width="75%" data-align="center">
<colgroup>
<col style="width: 20%" />
<col style="width: 20%" />
<col style="width: 20%" />
...
<tr>
<td width="20%"> </td>
<td style="text-align: center;" width="10%"><strong>json_each()</strong></td>
<td style="text-align: center;" width="10%"><strong>jsonb_each()</strong></td>
<td style="text-align: center;" width="10%"><strong>json_tree()</strong></td>
<td style="text-align: center;" width="10%"><strong>jsonb_tree()</strong></td>
</tr>
<tr>
<td width="20%">Recursively walk the substructure of the input JSON, or parse only a single layer</td>
<td style="text-align: center;" width="10%">single layer</td>
<td style="text-align: center;" width="10%">single layer</td>
<td style="text-align: center;" width="10%">recursive</td>
<td style="text-align: center;" width="10%">recursive</td>
</tr>
<tr>
<td width="20%">The "value" column returns text or JSONB for 'array' and 'object'</td>
<td style="text-align: center;" width="10%">text</td>
<td style="text-align: center;" width="10%">JSONB</td>
<td style="text-align: center;" width="10%">text</td>
<td style="text-align: center;" width="10%">JSONB</td>
</tr>
<tr>
<td width="20%">Supported since SQLite version...</td>
<td style="text-align: center;" width="10%">3.9.0<br />
(2015-10-14)</td>
<td style="text-align: center;" width="10%">3.51.0<br />
(2025-11-04)</td>
<td style="text-align: center;" width="10%">3.9.0<br />
(2015-10-14)</td>
<td style="text-align: center;" width="10%">3.51.0<br />
...
The two hidden columns, "json" and "root", are intended for input to the virtual table only. Use them for join constraints, including the implied join constraints that are constructed automatically by the [table-valued function](vtab.md#tabfunc2) syntax. If you try to return the values of the "json" or "root" columns, you might not get reliable answers.

### 4.24.1. Examples using json_each() and json_tree()
