---
title: SQLite Archive Files
source_url: https://www.sqlite.org/sqlar.html
source_path: sqlar.html
technology: sqlite
version: 3.53.4
license: Blessing
...
SQLite, in contrast, stores all content in a [single file on disk](fileformat2.md). That single file is something you can point to and say "this is the database". It behaves as an object. An SQLite database file can be copied, renamed, sent as an email attachment, passed as the argument for a POST HTTP request, or otherwise treated as other data objects such as an image, document, or media file.

Studies show that many applications already use SQLite as a container object. For example, [Kennedy](https://odin.cse.buffalo.edu/papers/2015/TPCTC-sqlite-final.pdf) (no relation to the SQLite developer with the same last name) reports that 14% of Android applications never write to their SQLite databases. It is believed that these applications are downloading entire databases from the cloud and then using the information locally as needed. In other words, the applications are using SQLite not so much as a database but as a queryable wire-transfer format.
...
2.  An SQLite Archive is transactional. Updates are atomic and durable, even if there are crashes or power losses in the middle of the update. Readers see a consistent and unchanging version of the content even if some other process is simultaneously updating the archive.
...
4.  An SQLite Archive can be queried using a high-level query language (SQL). Some examples:
    ...
    Questions like these (and countless others) can be answered without having to uncompress or extract any content.
...
2.  Since an SQLite database is a more general format (it is designed to do much more than simply store a bunch of files) it is not as compact as either the ZIP Archive or Tarball formats. An SQLite Archive is usually about 1% larger than the equivalent ZIP Archive. Tarballs are compressed as a single unit rather than compressing each file separately as is done by both SQLite and ZIP Archives. For these reason, Tarballs tend to be smaller than either ZIP or SQLite Archives.
    ...
    | ZIP Archive (using [zipfile](zipfile.md)) | 10,390,215 |
    | Tarball                                     |  9,781,109 |
...
<span id="cltools"></span>

# 4. Managing An SQLite Archive From The Command-Line

The recommended way of creating, updating, listing, and extracting an SQLite Archive is to use the [sqlite3.exe command-line shell](cli.md) for SQLite [version 3.23.0](releaselog/3_23_0.md) (2018-04-02) or later. This CLI supports the -A command-line option that allows easy management of SQLite Archives. The CLI for SQLite [version 3.22.0](releaselog/3_22_0.md) (2018-01-22) has the [.archive command](cli.md#sqlar) for managing SQLite Archives, but that requires interacting with the shell.
...
For usage hints and a summary of all options, simply give the [CLI](cli.md) the -A option with no additional arguments:

sqlite3 -A\

All of these commands work the same way if the filename argument is a ZIP Archive instead of an SQLite database.
...
Using the two routines above, it is simple for applications to insert new records into or extract existing records from an SQLite Archive. Insert a new record into an SQLite Archive using code like this:
