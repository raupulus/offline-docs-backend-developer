...
source_path: reference/compose-file/services.md
technology: docker
version: main
license: Apache-2.0
...
> [!NOTE]
>
> Unlike the `CMD` instruction in a Dockerfile, the `command` field doesn't automatically run within the context of the [`SHELL`](../../build/concepts/dockerfile.md#shell-form) instruction defined in the image. If your `command` relies on shell-specific features, such as environment variable expansion, you need to explicitly run it within a shell. For example:
>
> ```yaml
> command: /bin/sh -c 'echo "hello $$HOSTNAME"'
> ```

The value can also be a list, similar to the [exec-form syntax](../../build/concepts/dockerfile.md#exec-form)
used by the [Dockerfile](../../build/concepts/dockerfile.md#exec-form).
...
Consult the [network drivers documentation](../../engine/network/index.md) for more information.
...
For more information on `HEALTHCHECK`, see the [Dockerfile reference](../../build/concepts/dockerfile.md#healthcheck).
...
For more information container networks, see the [Docker Engine documentation](../../engine/network/index.md#container-networks).
...
> [!WARNING]
>
> If you do not specify a host IP (such as `127.0.0.1`), Docker binds to all interfaces (`0.0.0.0`), bypassing host firewall rules. This can expose the container directly to the internet if the host has a public IP address. For more information, see [Port publishing and mapping](../../engine/network/port-publishing.md).
...
For more information, see [Use lifecycle hooks](../../compose/how-tos/lifecycle.md).
...
For more information on using the `provider` attribute, see [Use provider services](../../compose/how-tos/provider-services.md).
...
> Compose now takes advantage of [Synchronized file shares](../../desktop/features/synchronized-file-sharing.md) and automatically creates file shares for bind mounts.
