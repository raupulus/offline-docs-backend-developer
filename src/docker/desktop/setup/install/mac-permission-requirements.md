...
source_path: manuals/desktop/setup/install/mac-permission-requirements.md
technology: docker
version: main
license: Apache-2.0
...
Privileged configurations are applied during the installation with the `--user` flag on the [install command](mac-install.md#install-from-the-command-line). In this case, you are not prompted to grant root privileges on the first run of Docker Desktop. Specifically, the `--user` flag:
