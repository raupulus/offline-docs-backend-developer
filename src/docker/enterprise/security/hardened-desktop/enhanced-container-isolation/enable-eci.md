...
source_path: manuals/enterprise/security/hardened-desktop/enhanced-container-isolation/enable-eci.md
technology: docker
version: main
license: Apache-2.0
...
- [Enforced sign-in](../../enforce-sign-in/index.md) (for administrators managing organization-wide settings only)
...
1. [Create or edit a setting policy](../settings-management/configure-admin-console.md).
1. Set **Enhanced Container Isolation** to **Always enabled**.

{{< /tab >}}
{{< tab name="JSON file" >}}

1. Create an [`admin-settings.json` file](../settings-management/configure-json-file.md) and add:
...
You can also configure [Docker socket mount permissions](config.md) for trusted images that need Docker API access.
...
- Review [Configure Docker socket exceptions and advanced settings](config.md).
- Review [Enhanced Container Isolation limitations](limitations.md).
