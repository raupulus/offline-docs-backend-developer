---
title: Security FAQs
description: Frequently asked questions about Docker single sign-on, identity providers,
  user management, SSO enforcement, and domain verification
source_repo: docker/docs
source_ref: main
source_commit: 083f66104
source_path: manuals/faqs/security.md
technology: docker
version: main
license: Apache-2.0
...
Yes, Docker supports your IdP's session timeout using a custom `dockerSessionMinutes` SAML attribute instead of the standard `SessionNotOnOrAfter` element. See [SSO attributes](../security/provisioning/index.md#sso-attributes) for more information.
...
Yes. Delete your existing IdP configuration in your Docker SSO connection, then [configure SSO using your new IdP](../security/authentication/single-sign-on/connect.md). If you had already turned on enforcement, turn off enforcement before updating the provider connection.
...
Contact your identity provider to retrieve a new X.509 certificate. Update with the new certificate in [SSO configuration settings](../security/authentication/single-sign-on/manage.md#manage-sso-connections) from Docker Home.
...
The SSO implementation uses Just-in-Time (JIT) provisioning by default. You can optionally turn off JIT in Docker Home if you turn on auto-provisioning using SCIM. See [Just-in-Time provisioning](../security/provisioning/just-in-time.md).
...
For detailed instructions, see [Configure single sign-on](../security/authentication/single-sign-on/connect.md).

### Is Docker SSO fully synced with the IdP?

Docker SSO provides Just-in-Time (JIT) provisioning by default. Users are provisioned when they authenticate with SSO. If users leave the organization, administrators must manually [remove the user](../accounts/organization/manage/members.md#remove-a-member-from-the-organization) from the organization.

[SCIM](../security/provisioning/scim/index.md) provides full synchronization with users and groups. When using SCIM, the recommended configuration is to turn off JIT so all auto-provisioning is handled by SCIM.
...
See [SSO authentication with JIT provisioning disabled](../security/provisioning/just-in-time.md#sso-authentication-with-jit-provisioning-disabled).
...
When SSO is enforced, [passwords are prevented from accessing the Docker CLI](../security/security-announcements.md#deprecation-of-password-logins-on-cli-when-sso-enforced). You must use a personal access token (PAT) for CLI authentication instead.

Each user must create a PAT to access the CLI. To learn how to create a PAT, see [Manage personal access tokens](../security/access-tokens/personal-access-tokens.md). Users who already used a PAT before SSO enforcement can continue using that PAT.

### How does SSO affect automation systems and CI/CD pipelines?

Before enforcing SSO, you must [create personal access tokens](../security/access-tokens/personal-access-tokens.md) to replace passwords in automation systems and CI/CD pipelines.
...
For more details, see [Enforce sign-in for Desktop](../enterprise/security/enforce-sign-in/index.md#enforcing-sign-in-versus-enforcing-single-sign-on-sso).
