...
source_url: https://www.postgresql.org/docs/17/vacuumlo.html
source_repo: https://github.com/postgres/postgres.git
source_ref: REL_17_STABLE
source_commit: 23088673d
source_path: vacuumlo.sgml
...
`-l limit`; `--limit=limit`  
Remove no more than \<limit\> large objects per transaction (default 1000). Since the server acquires a lock per LO removed, removing too many LOs in one transaction risks exceeding [???](#guc-max-locks-per-transaction). Set the limit to zero if you want all removals done in a single transaction.
