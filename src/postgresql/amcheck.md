---
title: amcheck — tools to verify table and index consistency
source_url: https://www.postgresql.org/docs/17/amcheck.html
source_repo: https://github.com/postgres/postgres.git
source_ref: REL_17_STABLE
source_commit: 23088673d
source_path: amcheck.sgml
...
`bt_index_check(index regclass, heapallindexed boolean, checkunique boolean) returns void` <span class="indexterm"></span>  
...
`bt_index_parent_check(index regclass, heapallindexed boolean, rootdescend boolean, checkunique boolean) returns void` <span class="indexterm"></span>  
...
`amcheck` can be effective at detecting various types of failure modes that [data checksums](#app-initdb-data-checksums) will fail to catch. These include:
...
- Structural inconsistencies caused by incorrect operator class implementations.
  ...
  If a problem like this arises, it may not affect each individual index that is ordered using an affected collation, simply because *indexed* values might happen to have the same absolute ordering regardless of the behavioral inconsistency. See [???](#locale) and [???](#collation) for further details about how PostgreSQL uses operating system locales and collations.
