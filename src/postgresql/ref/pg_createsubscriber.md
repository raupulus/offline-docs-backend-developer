...
source_url: https://www.postgresql.org/docs/17/app-pgcreatesubscriber.html
source_repo: https://github.com/postgres/postgres.git
source_ref: REL_17_STABLE
source_commit: 23088673d
source_path: ref/pg_createsubscriber.sgml
...
pg_createsubscriber creates a new logical replica from a physical standby server. All tables in the specified database are included in the [logical replication](#logical-replication) setup. A pair of publication and subscription objects are created for each database. It must be run at the target server.
...
The connection string to the publisher. For details see [???](#libpq-connstring).
...
9.  If the standby server was using [???](#guc-primary-slot-name), it has no use from now on so drop it.

10. If the standby server contains [failover replication slots](#logicaldecoding-replication-slots-synchronization), they cannot be synchronized anymore, so drop them.

11. Update the system identifier on the target server. The [???](#app-pgresetwal) is run to modify the system identifier. The target server is stopped as a `pg_resetwal` requirement.
