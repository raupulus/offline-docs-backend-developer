---
title: pg_rewind
description: synchronize a PostgreSQL data directory with another data directory that
  was forked from it
source_url: https://www.postgresql.org/docs/17/app-pgrewind.html
source_repo: https://github.com/postgres/postgres.git
source_ref: REL_17_STABLE
source_commit: 23088673d
source_path: ref/pg_rewind.sgml
...
pg_rewind requires that the target server either has the [???](#guc-wal-log-hints) option enabled in `postgresql.conf` or data checksums enabled when the cluster was initialized with initdb. Neither of these are currently on by default. [???](#guc-full-page-writes) must also be set to `on`, but is enabled by default.
