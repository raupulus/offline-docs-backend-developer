...
source_url: https://www.postgresql.org/docs/17/sql-createdatabase.html
source_repo: https://github.com/postgres/postgres.git
source_ref: REL_17_STABLE
source_commit: 23088673d
source_path: ref/create_database.sgml
...
\<locale\>  
Sets the default collation order and character classification in the new database. Collation affects the sort order applied to strings, e.g., in queries with `ORDER BY`, as well as the order used in indexes on text columns. Character classification affects the categorization of characters, e.g., lower, upper, and digit. Also sets the associated aspects of the operating system environment, `LC_COLLATE` and `LC_CTYPE`. The default is the same setting as the template database. See [???](#collation-managing-create-libc) and [???](#collation-managing-create-icu) for details.

Can be overridden by setting [varlistentry_title](#create-database-lc-collate), [varlistentry_title](#create-database-lc-ctype), [varlistentry_title](#create-database-builtin-locale), or [varlistentry_title](#create-database-icu-locale) individually.

If [varlistentry_title](#create-database-locale-provider) is `builtin`, then \<locale\> or \<builtin_locale\> must be specified and set to either `C` or `C.UTF-8`.

> [!TIP]
> The other locale settings [???](#guc-lc-messages), [???](#guc-lc-monetary), [???](#guc-lc-numeric), and [???](#guc-lc-time) are not fixed per database and are not set by this command. If you want to make them the default for a specific database, you can use `ALTER DATABASE ... SET`.

\<lc_collate\>  
Sets `LC_COLLATE` in the database server's operating system environment. The default is the setting of [varlistentry_title](#create-database-locale) if specified, otherwise the same setting as the template database. See below for additional restrictions.

If [varlistentry_title](#create-database-locale-provider) is `libc`, also sets the default collation order to use in the new database, overriding the setting [varlistentry_title](#create-database-locale).

\<lc_ctype\>  
Sets `LC_CTYPE` in the database server's operating system environment. The default is the setting of [varlistentry_title](#create-database-locale) if specified, otherwise the same setting as the template database. See below for additional restrictions.

If [varlistentry_title](#create-database-locale-provider) is `libc`, also sets the default character classification to use in the new database, overriding the setting [varlistentry_title](#create-database-locale).

\<builtin_locale\>  
Specifies the builtin provider locale for the database default collation order and character classification, overriding the setting [varlistentry_title](#create-database-locale). The [locale provider](#create-database-locale-provider) must be `builtin`. The default is the setting of [varlistentry_title](#create-database-locale) if specified; otherwise the same setting as the template database.

The locales available for the `builtin` provider are `C` and `C.UTF-8`.

\<icu_locale\>  
Specifies the ICU locale (see [???](#collation-managing-create-icu)) for the database default collation order and character classification, overriding the setting [varlistentry_title](#create-database-locale). The [locale provider](#create-database-locale-provider) must be ICU. The default is the setting of [varlistentry_title](#create-database-locale) if specified; otherwise the same setting as the template database.
...
\<locale_provider\>  
Specifies the provider to use for the default collation in this database. Possible values are `builtin`, `icu`<span class="indexterm"></span> (if the server was built with ICU support) or `libc`. By default, the provider is the same as that of the [varlistentry_title](#create-database-template). See [???](#locale-providers) for details.
...
See also [???](#sql-alterdatabase) for how to handle database collation version mismatches.
