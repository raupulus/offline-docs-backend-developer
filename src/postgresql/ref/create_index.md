...
source_url: https://www.postgresql.org/docs/17/sql-createindex.html
source_repo: https://github.com/postgres/postgres.git
source_ref: REL_17_STABLE
source_commit: 23088673d
source_path: ref/create_index.sgml
...
\[, ... \] ) \] \] \[ ASC \| DESC \] \[ NULLS { FIRST \| LAST } \] \[, ...\] ) \[ INCLUDE (

column_name

\[, ...\] ) \] \[ NULLS \[ NOT \] DISTINCT \] \[ WITH (
...
Additional restrictions apply when unique indexes are applied to partitioned tables; see [???](#sql-createtable).
...
`deduplicate_items` (`boolean`) <span class="indexterm"></span>  
Controls usage of the B-tree deduplication technique described in [???](#btree-deduplication). Set to `ON` or `OFF` to enable or disable the optimization. (Alternative spellings of `ON` and `OFF` are allowed as described in [???](#config-setting).) The default is `ON`.
...
`buffering` (`enum`) <span class="indexterm"></span>  
Determines whether the buffered build technique described in [???](#gist-buffering-build) is used to build the index. With `OFF` buffering is disabled, with `ON` it is enabled, and with `AUTO` it is initially disabled, but is turned on on-the-fly once the index size reaches [???](#guc-effective-cache-size). The default is `AUTO`. Note that if sorted build is possible, it will be used instead of buffered build unless `buffering=ON` is specified.

GIN indexes accept different parameters:

`fastupdate` (`boolean`) <span class="indexterm"></span>  
This setting controls usage of the fast update technique described in [???](#gin-fast-update). It is a Boolean parameter: `ON` enables fast update, `OFF` disables it. The default is `ON`.
...
Custom [???](#guc-gin-pending-list-limit) parameter. This value is specified in kilobytes.

BRIN indexes accept different parameters:

`pages_per_range` (`integer`) <span class="indexterm"></span>  
...
`autosummarize` (`boolean`) <span class="indexterm"></span>  
...
The system regularly collects statistics on all of a table's columns. Newly-created non-expression indexes can immediately use these statistics to determine an index's usefulness. For new expression indexes, it is necessary to run [`ANALYZE`](#sql-analyze) or wait for the [autovacuum daemon](#autovacuum) to analyze the table to generate statistics for these indexes.
