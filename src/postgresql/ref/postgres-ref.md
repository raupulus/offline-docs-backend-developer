...
source_url: https://www.postgresql.org/docs/17/app-postgres.html
source_repo: https://github.com/postgres/postgres.git
source_ref: REL_17_STABLE
source_commit: 23088673d
source_path: ref/postgres-ref.sgml
...
This can be used on a running server for most parameters. However, the server must be shut down for some runtime-computed parameters (e.g., [???](#guc-shared-memory-size), [???](#guc-shared-memory-size-in-huge-pages), and [???](#guc-wal-segment-size)).

This option is meant for other programs that interact with a server instance, such as [???](#app-pg-ctl), to query configuration parameter values. User-facing applications should instead use [`SHOW`](#sql-show) or the pg_settings view.
...
This option is deprecated since it does not allow access to the full functionality of [???](#guc-listen-addresses). It's usually better to set `listen_addresses` directly.
...
To quit the session, type EOF (<span class="keycombo">Control+D</span>, usually). If you've entered any text since the last command entry terminator, then EOF will be taken as a command entry terminator, and another EOF will be needed to exit.
