...
source_url: https://www.postgresql.org/docs/17/sql-altertable.html
source_repo: https://github.com/postgres/postgres.git
source_ref: REL_17_STABLE
source_commit: 23088673d
source_path: ref/alter_table.sgml
...
DROP DEFAULT ALTER \[ COLUMN \]

column_name

{ SET \| DROP } NOT NULL ALTER \[ COLUMN \]
...
DROP EXPRESSION \[ IF EXISTS \] ALTER \[ COLUMN \]
...
DROP IDENTITY \[ IF EXISTS \] ALTER \[ COLUMN \]
...
SET STORAGE { PLAIN \| EXTERNAL \| EXTENDED \| MAIN \| DEFAULT } ALTER \[ COLUMN \]
...
\[ DEFERRABLE \| NOT DEFERRABLE \] \[ INITIALLY DEFERRED \| INITIALLY IMMEDIATE \] VALIDATE CONSTRAINT
...
\| ALL \| USER \] ENABLE TRIGGER \[

trigger_name

\| ALL \| USER \] ENABLE REPLICA TRIGGER
...
DISABLE ROW LEVEL SECURITY ENABLE ROW LEVEL SECURITY FORCE ROW LEVEL SECURITY NO FORCE ROW LEVEL SECURITY CLUSTER ON

index_name

SET WITHOUT CLUSTER SET WITHOUT OIDS SET ACCESS METHOD {

new_access_method

\| DEFAULT } SET TABLESPACE

new_tablespace

SET { LOGGED \| UNLOGGED } SET (
...
\| CURRENT_ROLE \| CURRENT_USER \| SESSION_USER } REPLICA IDENTITY { DEFAULT \| USING INDEX
...
The rule firing mechanism is also affected by the configuration variable [???](#guc-session-replication-role), analogous to triggers as described above.
...
`SET ACCESS METHOD`  
This form changes the access method of the table by rewriting it using the indicated access method; specifying `DEFAULT` selects the access method set as the [???](#guc-default-table-access-method) configuration parameter. See [???](#tableam) for more information.
...
`SET { LOGGED | UNLOGGED }`  
This form changes the table from unlogged to logged or vice-versa (see [???](#sql-createtable-unlogged)). It cannot be applied to a temporary table.
