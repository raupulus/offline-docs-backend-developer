...
source_url: https://www.postgresql.org/docs/17/xindex.html
source_repo: https://github.com/postgres/postgres.git
source_ref: REL_17_STABLE
source_commit: 23088673d
source_path: xindex.sgml
...
Operator classes are associated with an index access method, such as [B-Tree](#btree) or [GIN](#gin). Custom index access method may be defined with [???](#sql-create-access-method). See [???](#indexam) for details.

The routines for an index method do not directly know anything about the data types that the index method will operate on. Instead, an operator class<span class="indexterm"></span> identifies the set of operations that the index method needs to use to work with a particular data type. Operator classes are so called because one thing they specify is the set of `WHERE`-clause operators that can be used with an index (i.e., can be converted into an index-scan qualification). An operator class can also specify some support function that are needed by the internal operations of the index method, but do not directly correspond to any `WHERE`-clause operator that can be used with the index.
...
B-Tree Strategies {#xindex-btree-strat-table}
...
Hash Strategies {#xindex-hash-strat-table}

GiST indexes are more flexible: they do not have a fixed set of strategies at all. Instead, the “consistency” support routine of each particular GiST operator class interprets the strategy numbers however it likes. As an example, several of the built-in GiST index operator classes index two-dimensional geometric objects, providing the “R-tree” strategies shown in [GiST Two-Dimensional Strategies](#xindex-rtree-strat-table). Four of these are true two-dimensional tests (overlaps, same, contains, contained by); four of them consider only the X direction; and the other four provide the same tests in the Y direction.
...
GiST Two-Dimensional “R-tree” Strategies {#xindex-rtree-strat-table}
...
SP-GiST Point Strategies {#xindex-spgist-point-strat-table}
...
GIN Array Strategies {#xindex-gin-array-strat-table}
...
BRIN Minmax Strategies {#xindex-brin-minmax-strat-table}
...
B-trees require a comparison support function, and allow four additional support functions to be supplied at the operator class author's option, as shown in [B-Tree Support Functions](#xindex-btree-support-table). The requirements for these support functions are explained further in [???](#btree-support-funcs).
...
B-Tree Support Functions {#xindex-btree-support-table}
...
Hash Support Functions {#xindex-hash-support-table}

GiST indexes have eleven support functions, six of which are optional, as shown in [GiST Support Functions](#xindex-gist-support-table). (For more information see [???](#gist).)
...
GiST Support Functions {#xindex-gist-support-table}

SP-GiST indexes have six support functions, one of which is optional, as shown in [SP-GiST Support Functions](#xindex-spgist-support-table). (For more information see [???](#spgist).)
...
SP-GiST Support Functions {#xindex-spgist-support-table}

GIN indexes have seven support functions, four of which are optional, as shown in [GIN Support Functions](#xindex-gin-support-table). (For more information see [???](#gin).)
...
GIN Support Functions {#xindex-gin-support-table}

BRIN indexes have five basic support functions, one of which is optional, as shown in [BRIN Support Functions](#xindex-brin-support-table). Some versions of the basic functions require additional support functions to be provided. (For more information see [???](#brin-extensibility).)
...
BRIN Support Functions {#xindex-brin-support-table}
