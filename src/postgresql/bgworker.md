...
source_url: https://www.postgresql.org/docs/17/bgworker.html
source_repo: https://github.com/postgres/postgres.git
source_ref: REL_17_STABLE
source_commit: 23088673d
source_path: bgworker.sgml
...
<span class="indexterm"></span> Requests shared memory access. This flag is required.

`BGWORKER_BACKEND_DATABASE_CONNECTION`  
<span class="indexterm"></span> Requests the ability to establish a database connection through which it can later run transactions and queries. A background worker using `BGWORKER_BACKEND_DATABASE_CONNECTION` to connect to a database must also attach shared memory using `BGWORKER_SHMEM_ACCESS`, or worker start-up will fail.
