---
title: postgres_fdw — access data stored in external PostgreSQL servers
source_url: https://www.postgresql.org/docs/17/postgres-fdw.html
source_repo: https://github.com/postgres/postgres.git
source_ref: REL_17_STABLE
source_commit: 23088673d
source_path: postgres-fdw.sgml
...
1.  Install the `postgres_fdw` extension using [???](#sql-createextension).
2.  Create a foreign server object, using [???](#sql-createserver), to represent each remote database you want to connect to. Specify connection information, except `user` and `password`, as options of the server object.
3.  Create a user mapping, using [???](#sql-createusermapping), for each database user you want to allow to access each foreign server. Specify the remote user name and password to use as `user` and `password` options of the user mapping.
4.  Create a foreign table, using [???](#sql-createforeigntable) or [???](#sql-importforeignschema), for each remote table you want to access. The columns of the foreign table must match the referenced remote table. You can, however, use table and/or column names different from the remote table's, if you specify the correct remote names as options of the foreign table object.
...
A foreign server using the `postgres_fdw` foreign data wrapper can have the same options that libpq accepts in connection strings, as described in [???](#libpq-paramkeywords), except that these options are not allowed or have special handling:
...
- [???](#guc-timezone) is set to `UTC`
- [???](#guc-datestyle) is set to `ISO`
- [???](#guc-intervalstyle) is set to `postgres`
- [???](#guc-extra-float-digits) is set to `3` for remote servers 9.0 and newer and is set to `2` for older versions
...
`postgres_fdw.application_name` (`string`) <span class="indexterm"></span>  
Specifies a value for [???](#guc-application-name) configuration parameter used when `postgres_fdw` establishes a connection to a foreign server. This overrides `application_name` option of the server object. Note that change of this parameter doesn't affect any existing connections until they are re-established.

`postgres_fdw.application_name` can be any string of any length and contain even non-ASCII characters. However when it's passed to and used as `application_name` in a foreign server, note that it will be truncated to less than `NAMEDATALEN` characters. Anything other than printable ASCII characters are replaced with [C-style hexadecimal escapes](#sql-syntax-strings-escape). See [???](#guc-application-name) for details.
...
| `%c` | Session ID on local server (see [???](#guc-log-line-prefix) for details) |
| `%C` | Cluster name on local server (see [???](#guc-cluster-name) for details) |
