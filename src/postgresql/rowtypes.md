...
source_url: https://www.postgresql.org/docs/17/rowtypes.html
source_repo: https://github.com/postgres/postgres.git
source_ref: REL_17_STABLE
source_commit: 23088673d
source_path: rowtypes.sgml
...
The \<composite_value\>`.*` syntax results in column expansion of this kind when it appears at the top level of a [`SELECT` output list](#queries-select-lists), a [`RETURNING` list](#dml-returning) in `INSERT`/`UPDATE`/`DELETE`/`MERGE`, a [`VALUES` clause](#queries-values), or a [row constructor](#sql-syntax-row-constructors). In all other contexts (including when nested inside one of those constructs), attaching `.*` to a composite value does not change the value, since it means “all columns” and so the same composite value is produced again. For example, if `somefunc()` accepts a composite-valued argument, these queries are the same:
...
This equivalence between functional notation and field notation makes it possible to use functions on composite types to implement “computed fields”. <span class="indexterm"></span> <span class="indexterm"></span> An application using the last query above wouldn't need to be directly aware that `somefunc` isn't a real column of the table.
