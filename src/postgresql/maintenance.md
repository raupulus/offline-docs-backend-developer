...
source_url: https://www.postgresql.org/docs/17/maintenance.html
source_repo: https://github.com/postgres/postgres.git
source_ref: REL_17_STABLE
source_commit: 23088673d
source_path: maintenance.sgml
...
The other main category of maintenance task is periodic “vacuuming” of the database. This activity is discussed in [Routine Vacuuming](#routine-vacuuming). Closely related to this is updating the statistics that will be used by the query planner, as discussed in [Updating Planner Statistics](#vacuum-for-statistics).

Another task that might need periodic attention is log file management. This is discussed in [Log File Maintenance](#logfile-maintenance).
...
Second, it allows PostgreSQL to answer some queries using only the index, without reference to the underlying table. Since PostgreSQL indexes don't contain tuple visibility information, a normal index scan fetches the heap tuple for each matching index entry, to check whether it should be seen by the current transaction. An [index-only scan](#indexes-index-only-scans), on the other hand, checks the visibility map first. If it's known that all tuples on the page are visible, the heap fetch can be skipped. This is most useful on large data sets where the visibility map can prevent disk accesses. The visibility map is vastly smaller than the heap, so it can easily be cached even when the heap is very large.
...
3.  Drop any old replication slots. Use [pg_replication_slots](#view-pg-replication-slots) to find slots where `age(xmin)` or `age(catalog_xmin)` is large. In many cases, such slots were created for replication to servers that no longer exist, or that have been down for a long time. If you drop a slot for a server that still exists and might still try to connect to that slot, that replica may need to be rebuilt.
...
where the vacuum base threshold is [???](#guc-autovacuum-vacuum-threshold), the vacuum scale factor is [???](#guc-autovacuum-vacuum-scale-factor), and the number of tuples is pg_class.reltuples.
