...
source_url: https://git-scm.com/doc/git-describe
source_repo: https://github.com/git/git.git
source_ref: master
source_commit: 339ab2a8f
source_path: git-describe.adoc
technology: git
version: master
license: GPL-2.0
...
By default (without `--all` or `--tags`) `git` `describe` only shows annotated tags. For more information about creating annotated tags see the `-a` and `-s` options to [git-tag](git-tag.md).
