...
source_url: https://git-scm.com/doc/git-help
source_repo: https://github.com/git/git.git
source_ref: master
source_commit: 339ab2a8f
source_path: git-help.adoc
technology: git
version: master
license: GPL-2.0
...
> ’git help’ <span id="-i|--info] [-m|--man] [-w|--web"></span> \[\<command\>\|\<doc\>\]\
> ’git help’ \[-g\|--guides\]\
> ’git help’ \[-c\|--config\]\
...
To display the [git](git.md) man page, use `git` `help` `git`.

This page can be displayed with ’git help help’ or `git` `help` `--help`.

OPTIONS ------- -a:: --all:: Print all the available commands on the standard output.
...
--config  
List all available configuration variables. This is a short summary of the list in [git-config](git-config.md).
...
In-repository file interfaces such as `.git/info/exclude` are documented here (see [gitrepository-layout](gitrepository-layout.md)), as well as in-tree configuration such as `.mailmap` (see [gitmailmap](gitmailmap.md)).

This section of the documentation also covers general or widespread user-interface conventions (e.g. [gitcli](gitcli.md)), and pseudo-configuration such as the file-based `.git/hooks/*` interface described in [githooks](githooks.md).
...
The web browser can be specified using the configuration variable `help.browser`, or `web.browser` if the former is not set. If neither of these config variables is set, the ’git web{litdd}browse’ helper script (called by ’git help’) will pick a suitable default. See [git-web](git-web.md){litdd}browse\[1\] for more information about this.
...
The `help.browser`, `web.browser` and `browser.<tool>.path` will also be checked if the ’web’ format is chosen (either by command-line option or configuration variable). See ’-w\|--web’ in the OPTIONS section above and [git-web](git-web.md){litdd}browse\[1\].
...
as they are probably more user specific than repository specific. See [git-config](git-config.md) for more information about this.
