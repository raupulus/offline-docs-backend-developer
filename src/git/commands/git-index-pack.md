...
source_url: https://git-scm.com/doc/git-index-pack
source_repo: https://github.com/git/git.git
source_ref: master
source_commit: 339ab2a8f
source_path: git-index-pack.adoc
technology: git
version: master
license: GPL-2.0
...
NAME ---- git-index-pack - Build pack index file for an existing packed archive
...
DESCRIPTION ----------- Reads a packed archive (.pack) from the specified file, builds a pack index file (.idx) for it, and optionally writes a reverse-index (.rev) for the specified pack. The packed archive, together with the pack index, can then be placed in the objects/pack/ directory of a Git repository.
...
--fix-thin  
Fix a "thin" pack produced by `git` `pack-objects` `--thin` (see [git-pack-objects](git-pack-objects.md) for details) by adding the excluded objects the deltified objects are based on to the pack. This option only makes sense in conjunction with --stdin.
...
--promisor\[=\<message\>\]  
Before committing the pack-index, create a .promisor file for this pack. Particularly helpful when writing a promisor pack with --fix-thin since the name of the pack is not final until the pack has been fully written. If a `<message>` is provided, then that content will be written to the .promisor file for future reference. See [partial clone](technical/partial-clone.html) for more information.
