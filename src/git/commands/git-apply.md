...
source_url: https://git-scm.com/doc/git-apply
source_repo: https://github.com/git/git.git
source_ref: master
source_commit: 339ab2a8f
source_path: git-apply.adoc
technology: git
version: master
license: GPL-2.0
...
This command applies the patch but does not create a commit. Use [git-am](git-am.md) to create commits from patches generated by [git-format-patch](git-format-patch.md) and/or received by email.

OPTIONS ------- `<patch>…`:: The files to read the patch from. `-` can be used to read from the standard input.
...
`--intent-to-add`  
When applying the patch only to the working tree, mark new files to be added to the index later (see `--intent-to-add` option in [git-add](git-add.md)). This option is ignored if `--index` or `--cached` are used, and has no effect outside a Git repository. Note that `--index` could be implied by other options such as `--3way`.
...
Don’t apply changes to files matching *\<path-pattern\>*. This can be useful when importing patchsets, where you want to exclude certain files or directories.

`--include=<path-pattern>`  
Apply changes to files matching the *\<path-pattern\>*. This can be useful when importing patchsets, where you want to include certain files or directories.
...
SUBMODULES ---------- If the patch contains any changes to submodules then `git` `apply` treats these changes as follows.
