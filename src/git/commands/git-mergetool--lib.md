...
source_url: https://git-scm.com/doc/git-mergetool--lib
source_repo: https://github.com/git/git.git
source_ref: master
source_commit: 339ab2a8f
source_path: git-mergetool--lib.adoc
technology: git
version: master
license: GPL-2.0
...
The ’git-mergetool{litdd}lib’ scriptlet is designed to be sourced (using `.`) by other shell scripts to set up functions for working with Git merge tools.
...
FUNCTIONS --------- get_merge_tool:: Returns a merge tool. The return code is 1 if we returned a guessed merge tool, else 0. ’\$GIT_MERGETOOL_GUI’ may be set to ’true’ to search for the appropriate guitool.
