...
source_url: https://git-scm.com/doc/git-bundle
source_repo: https://github.com/git/git.git
source_ref: master
source_commit: 339ab2a8f
source_path: git-bundle.adoc
technology: git
version: master
license: GPL-2.0
...
Git commands that fetch or otherwise "read" via protocols such as `ssh://` and [`https://`](https://) can also operate on bundle files. It is possible [git-clone](git-clone.md) a new repository from a bundle, to use [git-fetch](git-fetch.md) to fetch from one, and to list the references contained within it with [git-ls-remote](git-ls-remote.md). There’s no corresponding "write" support, i.e. a ’git push’ into a bundle is not supported.

BUNDLE FORMAT -------------

Bundles are `.pack` files (see [git-pack-objects](git-pack-objects.md)) with a header indicating what references are contained within the bundle.
...
Bundles created using revision exclusions are "thin packs" created using the `--thin` option to [git-pack-objects](git-pack-objects.md), and unbundled using the `--fix-thin` option to [git-index-pack](git-index-pack.md).
...
See [gitformat-bundle](gitformat-bundle.md) for more details and the discussion of "thin pack" in [gitformat-pack](gitformat-pack.md) for further details.
...
verify \<file\>  
Used to check that a bundle file is valid and will apply cleanly to the current repository. This includes checks on the bundle format itself as well as checking that the prerequisite commits exist and are fully linked in the current repository. Then, ’git bundle’ prints a list of missing commits, if any. Finally, information about additional capabilities, such as "object filter", is printed. See "Capabilities" in [gitformat-bundle](gitformat-bundle.md) for more information. The exit code is zero for success, but will be nonzero if the bundle file is invalid. If ’file’ is `-`, the bundle is read from stdin.
...
You can later recover that repository by using for example [git-clone](git-clone.md):
...
If you know up to what commit the intended recipient repository should have the necessary objects, you can use that knowledge to specify the prerequisites, giving a cut-off point to limit the revisions and objects that go in the resulting bundle. The previous example used the lastR2bundle tag for this purpose, but you can use any other options that you would give to the [git-log](git-log.md) command. Here are more examples:
