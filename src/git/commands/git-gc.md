...
NAME ---- git-gc - Cleanup unnecessary files and optimize the local repository
...
Running `git` `gc` manually should only be needed when adding objects to a repository without regularly running such porcelain commands, to do a one-off repository optimization, or e.g. to clean up a suboptimal mass-import. See the "PACKFILE OPTIMIZATION" section in [git-fast-import](git-fast-import.md) for more details on the import case.
...
--max-cruft-size=\<n\>  
When packing unreachable objects into a cruft pack, limit the size of new cruft packs to be at most `<n>` bytes. Overrides any value specified via the `gc.maxCruftSize` configuration. See the `--max-cruft-size` option of [git-repack](git-repack.md) for more.

--expire-to=\<dir\>  
When packing unreachable objects into a cruft pack, write a cruft pack containing pruned objects (if any) to the directory `<dir>`. This option only has an effect when used together with `--cruft`. See the `--expire-to` option of [git-repack](git-repack.md) for more information.
...
When the `--aggressive` option is supplied, [git-repack](git-repack.md) will be invoked with the `-f` flag, which in turn will pass `--no-reuse-delta` to [git-pack-objects](git-pack-objects.md). This will throw away any existing deltas and re-compute them, at the expense of spending much more time on the repacking.
...
Furthermore, supplying `--aggressive` will tweak the `--depth` and `--window` options passed to [git-repack](git-repack.md). See the `gc.aggressiveDepth` and `gc.aggressiveWindow` settings below. By using a larger window size we’re more likely to find more optimal deltas.
...
The ’git gc --auto’ command will run the ’pre-auto-gc’ hook. See [githooks](githooks.md) for more information.

SEE ALSO -------- [git-prune](git-prune.md) [git-reflog](git-reflog.md) [git-repack](git-repack.md) [git-rerere](git-rerere.md)
