...
source_url: https://git-scm.com/doc/git-fast-export
source_repo: https://github.com/git/git.git
source_ref: master
source_commit: 339ab2a8f
source_path: git-fast-export.adoc
technology: git
version: master
license: GPL-2.0
...
DESCRIPTION ----------- This program dumps the given revisions in a form suitable to be piped into ’git fast-import’.

You can use it as a human-readable bundle replacement (see [git-bundle](git-bundle.md)), or as a format that can be edited before being fed to ’git fast-import’ in order to do history rewrites (an ability relied on by tools like ’git filter-repo’).

OPTIONS ------- --progress=\<n\>:: Insert ’progress’ statements every \<n\> objects, to be shown by ’git fast-import’ during import.
...
While all the signatures of a commit are exported, an importer may choose to accept only some of them. For example [git-fast-import](git-fast-import.md) currently stores at most one signature per Git hash algorithm in each commit.
...
When asking to ’abort’ (which is the default), this program will die when encountering such a tag. With ’drop’ it will omit such tags from the output. With ’rewrite’, if the tagged object is a commit, it will rewrite the tag to tag an ancestor commit (via parent rewriting; see [git-rev-list](git-rev-list.md)).

-M  

-C  
Perform move and/or copy detection, as described in the [git-diff](git-diff.md) manual page, and use it to generate rename and copy commands in the output dump.
...
SEE ALSO -------- [git-fast-import](git-fast-import.md)
