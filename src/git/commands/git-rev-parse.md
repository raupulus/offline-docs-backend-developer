...
source_url: https://git-scm.com/doc/git-rev-parse
source_repo: https://github.com/git/git.git
source_ref: master
source_commit: 339ab2a8f
source_path: git-rev-parse.adoc
technology: git
version: master
license: GPL-2.0
...
--short\[=\<length\>\]  
Same as `--verify` but shortens the object name to a unique prefix with at least `length` characters. The minimum length is 4, the default is the effective value of the `core.abbrev` configuration variable (see [git-config](git-config.md)).
...
- Use `*` to mean that this option should not be listed in the usage generated for the `-h` argument. It’s shown for `--help-all` as documented in [gitcli](gitcli.md).
