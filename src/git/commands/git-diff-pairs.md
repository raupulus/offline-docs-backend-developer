...
description: Compare the content and mode of provided blob pairs
source_url: https://git-scm.com/doc/git-diff-pairs
source_repo: https://github.com/git/git.git
source_ref: master
source_commit: 339ab2a8f
source_path: git-diff-pairs.adoc
technology: git
version: master
license: GPL-2.0
...
NAME ---- git-diff-pairs - Compare the content and mode of provided blob pairs

SYNOPSIS --------

git diff-pairs -z \[\<diff-options\>\]

DESCRIPTION ----------- Show changes for file pairs provided on stdin. Input for this command must be in the NUL-terminated raw output format as generated by commands such as `git` `diff-tree` `-z` `-r` `--raw`. By default, the outputted diffs are computed and shown in the patch format when stdin closes.
