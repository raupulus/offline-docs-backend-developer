...
source_url: https://git-scm.com/doc/git-init
source_repo: https://github.com/git/git.git
source_ref: master
source_commit: 339ab2a8f
source_path: git-init.adoc
technology: git
version: master
license: GPL-2.0
...
ifndef  
with-breaking-changes\[\] (currently `master`, but this will change to `main` when Git 3.0 is released).

endif  
with-breaking-changes\[\]

ifdef  
with-breaking-changes\[\] `main`.

endif  
with-breaking-changes\[\] The default name can be customized via the `init.defaultBranch` configuration variable.
...
The default template directory includes some directory structure, suggested "exclude patterns" (see [gitignore](gitignore.md)), and sample hook files.

The sample hooks are all disabled by default. To enable one of the sample hooks rename it by removing its `.sample` suffix.

See [githooks](githooks.md) for more general info on hook execution.
