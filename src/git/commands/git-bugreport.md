...
source_url: https://git-scm.com/doc/git-bugreport
source_repo: https://github.com/git/git.git
source_ref: master
source_commit: 339ab2a8f
source_path: git-bugreport.adoc
technology: git
version: master
license: GPL-2.0
...
OPTIONS ------- -o \<path\>:: --output-directory \<path\>:: Place the resulting bug report file in `<path>` instead of the current directory.
...
Without `mode` specified, the diagnostic archive will contain the default set of statistics reported by `git` `diagnose`. An optional `mode` value may be specified to change which information is included in the archive. See [git-diagnose](git-diagnose.md) for the list of valid values for `mode` and details about their usage.
