...
source_url: https://git-scm.com/doc/git-shortlog
source_repo: https://github.com/git/git.git
source_ref: master
source_commit: 339ab2a8f
source_path: git-shortlog.adoc
technology: git
version: master
license: GPL-2.0
...
`--format[=<format>]`  
Instead of the commit subject, use some other information to describe each commit. *\<format\>* can be any string accepted by the `--format` option of `git` `log`, such as ’\* \[%h\] %s’. (See the ’PRETTY FORMATS’ section of [git-log](git-log.md).)

Each pretty-printed commit will be rewrapped before it is shown.

`--date=<format>`  
Show dates formatted according to the given date string. (See the `--date` option in the ’Commit Formatting’ section of [git-log](git-log.md)). Useful with `--group=format:<format>`.
...
- `trailer:<field>`, the *\<field\>* is interpreted as a case-insensitive commit message trailer (see [git-interpret-trailers](git-interpret-trailers.md)). For example, if your project uses `Reviewed-by` trailers, you might want to see who has been reviewing with `git` `shortlog` `-ns` `--group=trailer:reviewed-by`.
...
- `format:<format>`, any string accepted by the `--format` option of `git` `log`. (See the ’PRETTY FORMATS’ section of [git-log](git-log.md).)
