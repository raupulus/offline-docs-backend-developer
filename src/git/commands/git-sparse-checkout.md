...
source_url: https://git-scm.com/doc/git-sparse-checkout
source_repo: https://github.com/git/git.git
source_ref: master
source_commit: 339ab2a8f
source_path: git-sparse-checkout.adoc
technology: git
version: master
license: GPL-2.0
...
To ensure that adjusting the sparse-checkout settings within a worktree does not alter the sparse-checkout settings in other worktrees, the ’set’ subcommand will upgrade your repository config to use worktree-specific config if not already present. The sparsity defined by the arguments to the ’set’ subcommand are stored in the worktree-specific sparse-checkout file. See [git-worktree](git-worktree.md) and the documentation of `extensions.worktreeConfig` in [git-config](git-config.md) for more details.
...
The `--dry-run` option will list the directories that would be removed without deleting them. Running in this mode can be helpful to predict the behavior of the clean command or to determine which kinds of files are left in the sparse directories.
...
EXAMPLES -------- `git` `sparse-checkout` `set` `MY/DIR1` `SUB/DIR2`::
...
"Sparse checkout" allows populating the working directory sparsely. It uses the skip-worktree bit (see [git-update-index](git-update-index.md)) to tell Git whether a file in the working directory is worth looking at. If the skip-worktree bit is set, and the file is not present in the working tree, then its absence is ignored. Git will avoid populating the contents of those files, which makes a sparse checkout helpful when working in a repository with many files, but only a few are important to the current user.
...
The sparse-checkout file uses the same syntax as `.gitignore` files; see [gitignore](gitignore.md) for details. Here, though, the patterns are usually being used to select which files to include rather than which files to exclude. (However, it can get a bit confusing since gitignore-style patterns have negations defined by patterns which begin with a ’!’, so you can also select files to *not* include.)
...
See also the "Sparse Checkout" section of [git-read-tree](git-read-tree.md) to learn more about the gitignore-style patterns used in sparse checkouts.
...
[git-read-tree](git-read-tree.md) [gitignore](gitignore.md)
