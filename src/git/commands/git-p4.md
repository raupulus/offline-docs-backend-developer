...
DESCRIPTION ----------- This command provides a way to interact with p4 repositories using Git.
...
Clone \~\~\~\~~ Generally, ’git p4 clone’ is used to create a new Git directory from an existing p4 repository:
...
Sync \~\~\~~ As development continues in the p4 repository, those changes can be included in the Git repository using:
...
Unshelve \~\~\~\~\~\~\~~ Unshelving will take a shelved P4 changelist, and produce the equivalent git commit in the branch refs/remotes/p4-unshelved/\<changelist\>.
...
General options \~\~\~\~\~\~\~\~\~\~\~\~\~\~~ All commands except clone accept these options.

--git-dir \<dir\>  
Set the `GIT_DIR` environment variable. See [git](git.md).
...
--bare  
Perform a bare clone. See [git-clone](git-clone.md).
...
-M  
Detect renames. See [git-diff](git-diff.md). Renames will be represented in p4 using explicit ’move’ operations. There is no corresponding option to detect copies, but there are variables for both moves and copies.
...
DEPOT PATH SYNTAX ----------------- The p4 depot path argument to ’git p4 sync’ and ’git p4 clone’ can be one or more space-separated p4 depot paths, with an optional p4 revision specifier on the end:
...
PERFORMANCE ----------- The fast-import mechanism used by ’git p4’ creates one pack file for each invocation of ’git p4 sync’. Normally, Git garbage compression ([git-gc](git-gc.md)) automatically compresses these to fewer pack files, but explicit invocation of ’git repack -adf’ may improve performance.

CONFIGURATION VARIABLES ----------------------- The following config settings can be used to modify ’git p4’ behavior. They all are in the ’git-p4’ section.

General variables \~\~\~\~\~\~\~\~\~\~\~\~\~\~\~\~~ git-p4.user:: User specified as an option to all p4 commands, with ’-u \<user\>’. The environment variable `P4USER` can be used instead.
...
Submit variables \~\~\~\~\~\~\~\~\~\~\~\~\~\~\~~ git-p4.detectRenames:: Detect renames. See [git-diff](git-diff.md). This can be true, false, or a score as expected by ’git diff -M’.

git-p4.detectCopies  
Detect copies. See [git-diff](git-diff.md). This can be true, false, or a score as expected by ’git diff -C’.

git-p4.detectCopiesHarder  
Detect copies harder. See [git-diff](git-diff.md). A boolean.
