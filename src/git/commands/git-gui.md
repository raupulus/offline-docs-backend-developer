...
source_url: https://git-scm.com/doc/git-gui
source_repo: https://github.com/git/git.git
source_ref: master
source_commit: 339ab2a8f
source_path: git-gui.adoc
technology: git
version: master
license: GPL-2.0
...
COMMANDS -------- blame:: Start a blame viewer on the specified file on the given version (or working directory if not specified).
...
Examples -------- `git` `gui` `blame` `Makefile`::
...
<!-- -->
    ...
    you can use it as a GUI version of [git-mergetool](git-mergetool.md)
...
SEE ALSO -------- [gitk](gitk.md):: The Git repository browser. Shows branches, commit history and file differences. gitk is the utility started by ’git gui’’s Repository Visualize actions.

Other ----- ’git gui’ is actually maintained as an independent project, but stable versions are distributed as part of the Git suite for the convenience of end users.
