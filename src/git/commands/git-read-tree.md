...
source_url: https://git-scm.com/doc/git-read-tree
source_repo: https://github.com/git/git.git
source_ref: master
source_commit: 339ab2a8f
source_path: git-read-tree.adoc
technology: git
version: master
license: GPL-2.0
...
DESCRIPTION ----------- Reads the tree information given by \<tree-ish\> into the index, but does not actually **update** any of the files it "caches". (see: [git-checkout-index](git-checkout-index.md))
...
OPTIONS ------- -m:: Perform a merge, not just a read. The command will refuse to run if your index file has unmerged entries, indicating that you have not finished a previous merge you started.
...
MERGING ------- If `-m` is specified, ’git read-tree’ can perform 3 kinds of merge, a single tree merge if only 1 tree is given, a fast-forward merge with 2 trees, or a 3-way merge if 3 or more trees are provided.
...
Note: The skip-worktree capabilities in [git-update-index](git-update-index.md) and `read-tree` predated the introduction of [git-sparse-checkout](git-sparse-checkout.md). Users are encouraged to use the `sparse-checkout` command in preference to these plumbing commands for sparse-checkout/skip-worktree related needs. However, the information below might be useful to users trying to understand the pattern style used in non-cone mode of the `sparse-checkout` command.

"Sparse checkout" allows populating the working directory sparsely. It uses the skip-worktree bit (see [git-update-index](git-update-index.md)) to tell Git whether a file in the working directory is worth looking at.
...
SEE ALSO -------- [git-write-tree](git-write-tree.md), [git-ls-files](git-ls-files.md), [gitignore](gitignore.md), [git-sparse-checkout](git-sparse-checkout.md)
