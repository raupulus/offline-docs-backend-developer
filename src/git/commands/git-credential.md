...
source_url: https://git-scm.com/doc/git-credential
source_repo: https://github.com/git/git.git
source_ref: master
source_commit: 339ab2a8f
source_path: git-credential.adoc
technology: git
version: master
license: GPL-2.0
...
SYNOPSIS -------- ------------------ ’git credential’ (fill\|approve\|reject\|capability) ------------------
...
git-credential takes an "action" option on the command-line (one of `fill`, `approve`, or `reject`) and reads a credential description on stdin (see <a href="#IOFMT" class="cross-reference">INPUT/OUTPUT FORMAT</a>).
...
\
For example, if we want a password for [`https://example.com/foo.git`](https://example.com/foo.git), we might generate the following credential description (don’t forget the blank line at the end; it tells `git` `credential` that the application finished feeding all the information it has):
...
<div id="CAPA-IOFMT" data-wrapper="1">
