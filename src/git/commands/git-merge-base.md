...
source_url: https://git-scm.com/doc/git-merge-base
source_repo: https://github.com/git/git.git
source_ref: master
source_commit: 339ab2a8f
source_path: git-merge-base.adoc
technology: git
version: master
license: GPL-2.0
...
NAME ---- git-merge-base - Find as good common ancestors as possible for a merge
...
As a consequence, the ’merge base’ is not necessarily contained in each of the commit arguments if more than two commits are specified. This is different from [git-show-branch](git-show-branch.md) when used with the `--merge-base` option.
...
OPTIONS ------- -a:: --all:: Output all merge bases for the commits, instead of just one.
...
See also -------- [git-rev-list](git-rev-list.md), [git-show-branch](git-show-branch.md), [git-merge](git-merge.md)
