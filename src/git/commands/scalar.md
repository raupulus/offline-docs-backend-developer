...
-C \<directory\>  
Before running the subcommand, change the working directory. This option imitates the same option of [git](git.md).

-c \<key\>=\<value\>  
For the duration of running the specified subcommand, configure this setting. This option imitates the same option of [git](git.md).
...
scalar run ( all \| config \| commit-graph \| fetch \| loose-objects \| pack-files ) \[\<enlistment\>\]  
Run the given maintenance task (or all tasks, if `all` was specified). Except for `all` and `config`, this subcommand simply hands off to [git-maintenance](git-maintenance.md) (mapping `fetch` to `prefetch` and `pack-files` to `incremental-repack`).
...
This setting enables the `credential.useHttpPath` feature only for web URLs for Azure DevOps. This is important for users interacting with that service using multiple organizations and thus multiple credential tokens.
...
SEE ALSO -------- [git-clone](git-clone.md), [git-maintenance](git-maintenance.md).
