...
source_url: https://git-scm.com/doc/git-remote-ext
source_repo: https://github.com/git/git.git
source_ref: master
source_commit: 339ab2a8f
source_path: git-remote-ext.adoc
technology: git
version: master
license: GPL-2.0
...
DESCRIPTION ----------- This remote helper uses the specified ’\<command\>’ to connect to a remote Git server.

Data written to stdin of the specified ’\<command\>’ is assumed to be sent to a git:or git-upload-archive (depending on situation), and data read from stdout of \<command\> is assumed to be received from the same service.
...
This argument will not be passed to ’\<command\>’. Instead, it will cause the helper to start by sending git: the remote side with the service field set to an appropriate value and the repository field set to the rest of the argument. Default is not to send such a request.

This is useful if the remote side is git:some tunnel.

’%V’ (must be first characters in argument)  
This argument will not be passed to ’\<command\>’. Instead it sets the vhost field in the git: Default is not to send vhost in such request (if sent).
...
ssh -i /home/foo/.ssh/somekey user@host.example %S ’foo/repo’"  
...
git-server-alias foo %G/repo %Vfoo"  
...
git-server-alias foo %G/repo% with% spaces %Vfoo"  
