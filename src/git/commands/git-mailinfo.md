...
source_url: https://git-scm.com/doc/git-mailinfo
source_repo: https://github.com/git/git.git
source_ref: master
source_commit: 339ab2a8f
source_path: git-mailinfo.adoc
technology: git
version: master
license: GPL-2.0
...
NAME ---- git-mailinfo - Extracts patch and authorship from a single e-mail message
...
DESCRIPTION ----------- Reads a single e-mail message from the standard input, and writes the commit log message in \<msg\> file, and the patches in \<patch\> file. The author name, e-mail and e-mail subject are written out to the standard output to be used by ’git am’ to create a commit. It is usually not necessary to use this command directly. See [git-am](git-am.md) instead.

OPTIONS ------- -k:: Usually the program removes email cruft from the Subject: header line to extract the title line for the commit log message. This option prevents this munging, and is most useful when used to read back ’git format-patch -k’ output.\
