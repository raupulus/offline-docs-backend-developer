...
source_url: https://git-scm.com/doc/git-notes
source_repo: https://github.com/git/git.git
source_ref: master
source_commit: 339ab2a8f
source_path: git-notes.adoc
technology: git
version: master
license: GPL-2.0
...
To change which notes are shown by `git` `log`, see the `notes.displayRef` discussion in <a href="#CONFIGURATION" class="cross-reference">CONFIGURATION</a>.
...
OPTIONS ------- `-f`:: `--force`:: When adding notes to an object that already has notes, overwrite the existing notes (instead of aborting).
...
`--no-stripspace`  
Clean up whitespace. Specifically (see [git-stripspace](git-stripspace.md)):
...
- add a missing `n` to the last line if necessary.
