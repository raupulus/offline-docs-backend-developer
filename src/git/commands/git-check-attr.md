...
source_url: https://git-scm.com/doc/git-check-attr
source_repo: https://github.com/git/git.git
source_ref: master
source_commit: 339ab2a8f
source_path: git-check-attr.adoc
technology: git
version: master
license: GPL-2.0
...
DESCRIPTION ----------- For every pathname, this command will list if each attribute is ’unspecified’, ’set’, or ’unset’ as a gitattribute on that pathname.

OPTIONS ------- -a:: --all:: List all attributes that are associated with the specified paths. If this option is used, then ’unspecified’ attributes will not be included in the output.
...
Buffering happens as documented under the `GIT_FLUSH` option in [git](git.md). The caller is responsible for avoiding deadlocks caused by overfilling an input buffer or reading from an empty output buffer.
