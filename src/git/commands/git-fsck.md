...
source_url: https://git-scm.com/doc/git-fsck
source_repo: https://github.com/git/git.git
source_ref: master
source_commit: 339ab2a8f
source_path: git-fsck.adoc
technology: git
version: master
license: GPL-2.0
...
NAME ---- git-fsck - Verifies the connectivity and validity of the objects in the database
...
OPTIONS ------- \<object\>:: An object to treat as the head of an unreachability trace.\
...
--name-objects  
When displaying names of reachable objects, in addition to the SHA-1 also display a name that describes **how** they are reachable, compatible with [git-rev-parse](git-rev-parse.md), e.g. `HEAD@{1234567890}~25^2:src/`.
...
--no-references  
Control whether to check the references database consistency via ’git refs verify’. See [git-refs](git-refs.md) for details. The default is to check the references database.
...
If core.commitGraph is true, the commit-graph file will also be inspected using ’git commit-graph verify’. See [git-commit-graph](git-commit-graph.md).
...
<div class="included" data-path="fsck-msgids.adoc">
