...
source_url: https://git-scm.com/doc/git-tag
source_repo: https://github.com/git/git.git
source_ref: master
source_commit: 339ab2a8f
source_path: git-tag.adoc
technology: git
version: master
license: GPL-2.0
...
NAME ---- git-tag - Create, list, delete or verify tags
...
OPTIONS ------- `-a`:: `--annotate`:: Make an unsigned, annotated tag object

`-s`  

`--sign`  
Make a cryptographically signed tag, using the default signing key. The signing backend used depends on the `gpg.format` configuration variable. The default key is determined by the backend. For GPG, it’s based on the committer’s email address, while for SSH it may be a specific key file or agent identity. See [git-config](git-config.md).
...
`--local-user=<key-id>`  
Make a cryptographically signed tag using the given key. The format of the \<key-id\> and the backend used depend on the `gpg.format` configuration variable. See [git-config](git-config.md).
...
`--sort=<key>`  
Sort based on the key given. Prefix `-` to sort in descending order of the value. You may use the `--sort=<key>` option multiple times, in which case the last *\<key\>* becomes the primary key. Also supports “version:refname” or “v:refname” (tag names are treated as versions). The “version:refname” sort order can also be affected by the “versionsort.suffix” configuration variable. The keys supported are the same as those in `git` `for-each-ref`. Sort order defaults to the value configured for the `tag.sort` variable if it exists, or lexicographic order otherwise. See [git-config](git-config.md).
...
`--trailer` `<token>[(=|:)<value>]`  
Specify a (*\<token\>*, *\<value\>*) pair that should be applied as a trailer. (e.g. `git` `tag` `--trailer` `"Custom-Key:` `value"` will add a "Custom-Key" trailer to the tag message.) The `trailer.*` configuration variables ([git-interpret-trailers](git-interpret-trailers.md)) can be used to define if a duplicated trailer is omitted, where in the run of trailers each trailer would appear, and other details. The trailers can be extracted in `git` `tag` `--list`, using `--format="%(trailers)"` placeholder.
...
`--create-reflog`  
Create a reflog for the tag. To globally enable reflogs for tags, see `core.logAllRefUpdates` in [git-config](git-config.md). The negated form `--no-create-reflog` only overrides an earlier `--create-reflog`, but currently does not negate the setting of `core.logAllRefUpdates`.

`--format=<format>`  
A string that interpolates `%(fieldname)` from a tag ref being shown and the object it points at. The format is the same as that of [git-for-each-ref](git-for-each-ref.md). When unspecified, defaults to `%(refname:strip=2)`.

*\<tagname\>*  
The name of the tag to create, delete, or describe. The new tag name must pass all checks defined by [git-check-ref-format](git-check-ref-format.md). Some of these checks may restrict the characters allowed in a tag name.
...
CONFIGURATION ------------- By default, `git` `tag` in sign-with-default mode (`-s`) will use your committer identity (of the form `Your` `Name` `<your@email.address>`) to find a key. If you want to use a different default key, you can specify it in the repository configuration as follows:

    [user]
        signingKey = <key-id>

The signing backend can be chosen via the `gpg.format` configuration variable, which defaults to `openpgp`. See [git-config](git-config.md) for a list of other supported formats.

The path to the program used for each signing backend can be specified with the `gpg.<format>.program` configuration variable. For the `openpgp` backend, `gpg.program` can be used as a synonym for `gpg.openpgp.program`. See [git-config](git-config.md) for details.

`pager.tag` is only respected when listing tags, i.e., when `-l` is used or implied. The default is to use a pager.

See [git-config](git-config.md) for more details and other configuration variables.
...
1.  The insane thing. You really want to call the new version "X" too, ’even though’ others have already seen the old one. So just use `git` `tag` `-f` again, as if you hadn’t already published the old one.
...
SEE ALSO -------- [git-check-ref-format](git-check-ref-format.md). [git-config](git-config.md).
