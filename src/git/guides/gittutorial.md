...
source_url: https://git-scm.com/doc/gittutorial
source_repo: https://github.com/git/git.git
source_ref: master
source_commit: 339ab2a8f
source_path: gittutorial.adoc
technology: git
version: master
license: GPL-2.0
...
NAME ---- gittutorial - A tutorial introduction to Git
...
(The complete configuration created by `git` `clone` is visible using `git` `config` `list`, and the [git-config](git-config.md) man page explains the meaning of each option.)
...
Alternatively, Git has a native protocol, or can use http; see [git-pull](git-pull.md) for details.

Git can also be used in a CVS-like mode, with a central repository that various users push changes to; see [git-push](git-push.md) and [gitcvs-migration](gitcvs-migration.md).
...
you can refer to `1b2e1d63ff` by the name `v2.5`. If you intend to share this name with other people (for example, to identify a release version), you should create a "tag" object, and perhaps sign it; see [git-tag](git-tag.md) for details.
...
Part two of this tutorial explains the object database, the index file, and a few other odds and ends that you’ll need to make the most of Git. You can find it at [gittutorial-2](gittutorial-2.md).

If you don’t want to continue with that right away, a few other digressions that may be interesting at this point are:

- [git-format-patch](git-format-patch.md), [git-am](git-am.md): These convert series of git commits into emailed patches, and vice versa, useful for projects such as the Linux kernel which rely heavily on emailed patches.

<!-- -->

- [git-bisect](git-bisect.md): When there is a regression in your project, one way to track down the bug is by searching through the history to find the exact commit that’s to blame. `git` `bisect` can help you perform a binary search for that commit. It is smart enough to perform a close-to-optimal search even in the case of complex non-linear history with lots of merged branches.

<!-- -->

- [gitworkflows](gitworkflows.md): Gives an overview of recommended workflows.

<!-- -->

- [giteveryday](giteveryday.md): Everyday Git with 20 Commands Or So.

<!-- -->

- [gitcvs-migration](gitcvs-migration.md): Git for CVS users.

SEE ALSO -------- [gittutorial-2](gittutorial-2.md), [gitcvs-migration](gitcvs-migration.md), [gitcore-tutorial](gitcore-tutorial.md), [gitglossary](gitglossary.md), [git-help](git-help.md), [gitworkflows](gitworkflows.md), [giteveryday](giteveryday.md), [The Git User’s Manual](user-manual.html)
