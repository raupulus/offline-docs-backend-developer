...
source_url: https://git-scm.com/doc/gitworkflows
source_repo: https://github.com/git/git.git
source_ref: master
source_commit: 339ab2a8f
source_path: gitworkflows.adoc
technology: git
version: master
license: GPL-2.0
...
NAME ---- gitworkflows - An overview of recommended workflows with Git
...
As a general rule, you should try to split your changes into small logical steps, and commit each of them. They should be consistent, working independently of any later commits, pass the test suite, etc. This makes the review process much easier, and the history much more useful for later inspection and analysis, for example with [git-blame](git-blame.md) and [git-bisect](git-bisect.md).

To achieve this, try to split your work into small steps from the very beginning. It is always easier to squash a few commits together than to split one big commit into several. Don’t be afraid of making too small or imperfect steps along the way. You can always go back later and edit the commits with `git` `rebase` `--interactive` before you publish them. You can use `git` `stash` `push` `--keep-index` to run the test suite independent of other uncommitted changes; see the EXAMPLES section of [git-stash](git-stash.md).

MANAGING BRANCHES -----------------

There are two main tools that can be used to include changes from one branch on another: [git-merge](git-merge.md) and [git-cherry-pick](git-cherry-pick.md).
...
<div class="example" data-caption="Rule: ">
...
- If you find you forked off the wrong branch and want to move it "back in time", use [git-rebase](git-rebase.md).

Note that the last point clashes with the other two: a topic that has been merged elsewhere should not be rebased. See the section on RECOVERING FROM UPSTREAM REBASE in [git-rebase](git-rebase.md).
...
<div class="example" data-caption="Recipe: ">
...
- [git-push](git-push.md) copies your branches to a remote repository, usually to one that can be read by all involved parties;

<!-- -->

- [git-fetch](git-fetch.md) that copies remote branches to your repository; and

<!-- -->

- [git-pull](git-pull.md) that does fetch and merge in one go.
...
You will still have to tell people by other means, such as mail. (Git provides the [git-request-pull](git-request-pull.md) to send preformatted pull requests to upstream maintainers to simplify this task.)
...
If you are a contributor that sends changes upstream in the form of emails, you should use topic branches as usual (see above). Then use [git-format-patch](git-format-patch.md) to generate the corresponding emails (highly recommended over manually formatting them because it makes the maintainer’s life easier).
...
See the [git-format-patch](git-format-patch.md) and [git-send-email](git-send-email.md) manpages for further usage notes.
...
One feature worth pointing out is the three-way merge, which can help if you get conflicts: `git` `am` `-3` will use index information contained in patches to figure out the merge base. See [git-am](git-am.md) for other options.

SEE ALSO -------- [gittutorial](gittutorial.md), [git-push](git-push.md), [git-pull](git-pull.md), [git-merge](git-merge.md), [git-rebase](git-rebase.md), [git-format-patch](git-format-patch.md), [git-send-email](git-send-email.md), [git-am](git-am.md)
