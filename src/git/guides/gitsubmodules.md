...
source_url: https://git-scm.com/doc/gitsubmodules
source_repo: https://github.com/git/git.git
source_ref: master
source_commit: 339ab2a8f
source_path: gitsubmodules.adoc
technology: git
version: master
license: GPL-2.0
...
Assuming the submodule has a Git directory at `$GIT_DIR/modules/foo/` and a working directory at `path/to/bar/`, the superproject tracks the submodule via a `gitlink` entry in the tree at `path/to/bar` and an entry in its `.gitmodules` file (see [gitmodules](gitmodules.md)) of the form `submodule.foo.path` `=` `path/to/bar`.
