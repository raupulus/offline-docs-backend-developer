...
description: Helper programs to interact with remote repositories
source_url: https://git-scm.com/doc/gitremote-helpers
source_repo: https://github.com/git/git.git
source_ref: master
source_commit: 339ab2a8f
source_path: gitremote-helpers.adoc
technology: git
version: master
license: GPL-2.0
...
NAME ---- gitremote-helpers - Helper programs to interact with remote repositories
...
A URL of the form ’\<transport\>  
\<address\>’ explicitly instructs Git to invoke ’git remote-\<transport\>’ with ’\<address\>’ as the second argument. If such a URL is encountered directly on the command line, the first argument is ’\<address\>’, and if it is encountered in a configured remote, the first argument is the name of that remote.
...
’export-marks’ \<file\>  
This modifies the ’export’ capability, instructing Git to dump the internal marks table to \<file\> when complete. For details, read up on `--export-marks=<file>` in [git-fast-export](git-fast-export.md).

’import-marks’ \<file\>  
This modifies the ’export’ capability, instructing Git to load the marks specified in \<file\> before processing any input. For details, read up on `--import-marks=<file>` in [git-fast-export](git-fast-export.md).

’signed-tags’  
This modifies the ’export’ capability, instructing Git to pass `--signed-tags=verbatim` to [git-fast-export](git-fast-export.md). In the absence of this capability, Git will use `--signed-tags=warn-strip`.
...
SEE ALSO -------- [git-remote](git-remote.md)
