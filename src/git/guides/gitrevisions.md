...
source_url: https://git-scm.com/doc/gitrevisions
source_repo: https://github.com/git/git.git
source_ref: master
source_commit: 339ab2a8f
source_path: gitrevisions.adoc
technology: git
version: master
license: GPL-2.0
...
NAME ---- gitrevisions - Specifying revisions and ranges for Git

SYNOPSIS -------- gitrevisions

DESCRIPTION -----------

Many Git commands take revision parameters as arguments. Depending on the command, they denote a specific commit or, for commands which walk the revision graph (such as [git-log](git-log.md)), all commits which are reachable from that commit. For commands that walk the revision graph one can also specify a range of revisions explicitly.

In addition, some Git commands (such as [git-show](git-show.md) and [git-push](git-push.md)) can also take revision parameters which denote other objects than commits, e.g. blobs ("files") or trees ("directories of files").

<div class="included" data-path="revisions.adoc">
...
1.  otherwise, ’refs/\<refname\>’ if it exists;

<!-- -->

1.  otherwise, ’refs/tags/\<refname\>’ if it exists;
...
1.  otherwise, ’refs/remotes/\<refname\>’ if it exists;
