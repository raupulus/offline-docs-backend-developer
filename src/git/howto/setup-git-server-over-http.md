---
title: setup-git-server-over-http
source_url: https://git-scm.com/doc/setup-git-server-over-http
source_repo: https://github.com/git/git.git
source_ref: master
source_commit: 339ab2a8f
source_path: howto/setup-git-server-over-http.adoc
technology: git
version: master
license: GPL-2.0
...
From: Rutger Nijlunsing \<[rutger@nospam.com](rutger@nospam.com)\> Subject: Setting up a Git repository which can be pushed into and pulled from over HTTP(S). Date: Thu, 10 Aug 2006 22:00:26 +0200 Content-type: text/asciidoc

How to setup Git server over http =================================

> [!NOTE]
> This document is from 2006. A lot has happened since then, and this document is now relevant mainly if your web host is not CGI capable. Almost everyone else should instead look at [git-http-backend](git-http-backend.md).
...
Now go to [http://\<username](http://<username)\>@\<servername\>/my-new-repo.git in your browser to check whether it asks for a password and accepts the right password.
...
If you’re into Windows, from XP onwards Internet Explorer supports WebDAV. For this, do Internet Explorer → Open Location → [http://\<servername](http://<servername)\>/my-new-repo.git \[x\] Open as webfolder → login .
...
If you have to access the WebDAV server from behind an HTTP(S) proxy, set the variable ’all_proxy’ to [`http://proxy-host.com:port`](http://proxy-host.com:port), or [`http://login-on-proxy:passwd-on-proxy@proxy-host.com:port`](http://login-on-proxy:passwd-on-proxy@proxy-host.com:port). See ’man curl’ for details.
...
Authors Johannes Schindelin \<[Johannes.Schindelin@gmx.de](Johannes.Schindelin@gmx.de)\> Rutger Nijlunsing \<[git@wingding.demon](git@wingding.demon).nl\> Matthieu Moy \<[Matthieu.Moy@imag.fr](Matthieu.Moy@imag.fr)\>
