...
description: Calcula la diferencia entre dos arrays asociativos, compara los datos
  y los índices con una función de retorno
source_url: https://www.php.net/manual/es/function.array-udiff-uassoc.php
source_repo: https://github.com/php/doc-es.git
source_ref: master
source_commit: 954a0d911
source_path: reference/array/functions/array-udiff-uassoc.xml
technology: php
version: master
license: CC-BY-3.0
...
Se debe tener en cuenta que las claves se utilizan en las comparaciones, a diferencia de `array_diff` y `array_udiff`.
...
`key_compare_func`  
La comparación de las claves (índices) se realiza mediante la función de retorno `key_compare_func`. Este comportamiento difiere del de `array_udiff_assoc`, ya que esta última utiliza una función interna para comparar los índices.

## Valores devueltos

Devuelve un `array` que contiene todas las claves del array `array` que no están presentes en ningún otro argumento.

## Ejemplos

Ejemplo con `array_udiff_uassoc`
...
En nuestro ejemplo, se observa que el par `"1" => new cr(4)` está presente en ambos arrays y, por lo tanto, ausente del array resultante. Se debe tener en cuenta que se deben proporcionar dos funciones de retorno.

## Notas

> [!NOTE]
> Se debe tener en cuenta que esta función solo verifica una dimensión de un array multidimensional. Por supuesto, se puede probar una dimensión particular utilizando, por ejemplo, `array_udiff_uassoc($array1[0], $array2[0], "data_compare_func", "key_compare_func");`.
