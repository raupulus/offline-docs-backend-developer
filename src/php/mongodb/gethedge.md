---
title: MongoDB\Driver\ReadPreference::getHedge
description: Devuelve la opción "hedge" del ReadPreference
source_url: https://www.php.net/manual/es/mongodb-driver-readpreference.gethedge.php
source_repo: https://github.com/php/doc-es.git
source_ref: master
source_commit: 954a0d911
source_path: reference/mongodb/mongodb/driver/readpreference/gethedge.xml
technology: php
version: master
license: CC-BY-3.0
retrieved_at: '2026-08-02'
section: mongodb
translation_status: ready
translation_reviewed: false
translation_revision: 9f4cb232d
order: 50880
---

MongoDB\Driver\ReadPreference::getHedge

Devuelve la opción "hedge" del ReadPreference

## Descripción

```php
final public MongoDB\Driver\ReadPreference::getHedge(): object
```php

## Parámetros

Esta función no contiene ningún parámetro.

## Valores devueltos

Devuelve la opción "hedge" del ReadPreference.

## Errores/Excepciones

Lanza una excepción

MongoDB\Driver\Exception\InvalidArgumentException

en caso de error durante el análisis de un argumento.

## Véase también

Referencia de Read Preference
