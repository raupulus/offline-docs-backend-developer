---
title: Gmagick::getimageformat
description: Devuelve el formato de una imagen en particular de una secuencia
source_url: https://www.php.net/manual/es/gmagick.getimageformat.php
source_repo: https://github.com/php/doc-es.git
source_ref: master
source_commit: 954a0d911
source_path: reference/gmagick/gmagick/getimageformat.xml
technology: php
version: master
license: CC-BY-3.0
retrieved_at: '2026-08-02'
section: gmagick
translation_status: ready
translation_revision: 35752f072
order: 26860
---

Gmagick::getimageformat

Devuelve el formato de una imagen en particular de una secuencia

## Descripción

```php
public Gmagick::getimageformat(): string
```php

Devuelve el formato de una imagen en particular de una secuencia.

## Parámetros

Esta función no contiene ningún parámetro.

## Valores devueltos

Devuelve una cadena que contiene el formato de imagen si se tuvo éxito.

## Errores/Excepciones

Emite una excepción `GmagickException` en caso de error.
