---
title: Gmagick::getimagemattecolor
description: Devuelve el color mate de la imagen
source_url: https://www.php.net/manual/es/gmagick.getimagemattecolor.php
source_repo: https://github.com/php/doc-es.git
source_ref: master
source_commit: 954a0d911
source_path: reference/gmagick/gmagick/getimagemattecolor.xml
technology: php
version: master
license: CC-BY-3.0
retrieved_at: '2026-08-02'
section: gmagick
translation_status: ready
translation_reviewed: false
translation_revision: 35752f072
order: 26950
---

Gmagick::getimagemattecolor

Devuelve el color mate de la imagen

## Descripción

```php
public Gmagick::getimagemattecolor(): GmagickPixel
```php

Devuelve un objeto GmagickPixel si se tuvo éxtito. Lanza un excepción GmagickException si se produjo un error.

## Parámetros

Esta función no contiene ningún parámetro.

## Valores devueltos

Devuelve un objeto GmagickPixel si se tuvo éxito.

## Errores/Excepciones

Emite una excepción `GmagickException` en caso de error.
