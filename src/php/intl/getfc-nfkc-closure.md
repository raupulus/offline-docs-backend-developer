---
title: IntlChar::getFC_NFKC_Closure
description: Devuelve la propiedad FC_NFKC_Closure para un punto de código
source_url: https://www.php.net/manual/es/intlchar.getfc-nfkc-closure.php
source_repo: https://github.com/php/doc-es.git
source_ref: master
source_commit: 954a0d911
source_path: reference/intl/intlchar/getfc-nfkc-closure.xml
technology: php
version: master
license: CC-BY-3.0
retrieved_at: '2026-08-02'
section: intl
translation_status: ready
translation_reviewed: true
translation_revision: 1976eae0d
order: 40840
---

IntlChar::getFC_NFKC_Closure

Devuelve la propiedad FC_NFKC_Closure para un punto de código

## Descripción

```php
public static IntlChar::getFC_NFKC_Closure(int $codepoint): string
```php

Devuelve la propiedad FC_NFKC_Closure para un carácter.

## Parámetros

`codepoint`  
El valor `int` del punto de código (por ejemplo, `0x2603` para *U+2603 SNOWMAN*), o el carácter codificado como un `string` UTF-8 (por ejemplo, `"\u{2603}"`)

## Valores devueltos

Devuelve la propiedad FC_NFKC_Closure para el `codepoint`, o una cadena vacía si no existe. Devuelve `null` o `false` en caso de error.

## Ejemplos

Probar diferentes puntos de código

```
    
<?php
var_dump(IntlChar::getFC_NFKC_Closure("\u{2121}"));
var_dump(IntlChar::getFC_NFKC_Closure("\u{1D2D}"));
?>

   
```php

El ejemplo anterior mostrará:

        
    string(3) "tel"
    string(2) "æ"
