---
title: UI\Menu::appendSeparator
description: Añade separador de elementos del menú
source_url: https://www.php.net/manual/es/ui-menu.appendseparator.php
source_repo: https://github.com/php/doc-es.git
source_ref: master
source_commit: 954a0d911
source_path: reference/ui/ui/menu/appendseparator.xml
technology: php
version: master
license: CC-BY-3.0
retrieved_at: '2026-08-02'
section: ui
translation_status: ready
translation_reviewed: false
translation_revision: 2735ac5fa
order: 97780
---

UI\Menu::appendSeparator

Añade separador de elementos del menú

## Descripción

```php
public UI\Menu::appendSeparator()
```php

Se añadirá un separador

## Parámetros

Esta función no contiene ningún parámetro.
