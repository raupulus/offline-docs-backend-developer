...
source_url: https://www.php.net/manual/es/function.eio-truncate.php
source_repo: https://github.com/php/doc-es.git
source_ref: master
source_commit: 954a0d911
source_path: reference/eio/functions/eio-truncate.xml
technology: php
version: master
license: CC-BY-3.0
...
```php
eio_truncate(string $path, [int $offset], [int $pri], [callable $callback], [mixed $data]): resource
```php

`eio_truncate` hace que el fichero nominado por `path` sea truncado al tamaño de exactamente `length` bytes
...
`eio_truncate` devuelve un recurso de petición en caso de éxito, o `false` si ocurre un error.
