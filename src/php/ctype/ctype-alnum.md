---
title: ctype_alnum
description: Chequear posibles caracteres alfanuméricos
source_url: https://www.php.net/manual/es/function.ctype-alnum.php
source_repo: https://github.com/php/doc-es.git
source_ref: master
source_commit: 954a0d911
source_path: reference/ctype/functions/ctype-alnum.xml
technology: php
version: master
license: CC-BY-3.0
retrieved_at: '2026-08-02'
section: ctype
translation_status: ready
translation_reviewed: false
translation_revision: e20e74073
order: 8450
---

ctype_alnum

Chequear posibles caracteres alfanuméricos

## Descripción

```php
ctype_alnum(mixed $text): bool
```php

Chequea si todos los caracteres en la `string` entregada, `texto`, son alfanuméricos.

## Parámetros

`text`  
La cadena de prueba.

> [!NOTE]
> Si se proporciona un `int` entre -128 y 255 inclusive, este se interpreta como el valor ASCII de un solo carácter (a los valores negativos se les añade 256 para permitir caracteres del rango ASCII extendido). Cualquier otro entero se interpreta como un string que contiene los dígitos decimales del entero.

> [!WARNING]
> A partir de PHP 8.1.0, pasar un argumento que no sea string está obsoleto. En el futuro, el argumento se interpretará como un string en lugar de un punto de código ASCII. Según el comportamiento deseado, el argumento debería convertirse a `string` o se debería realizar una llamada explícita a `chr`.

## Valores devueltos

Devuelve `true` si cada caracter de `texto` es o bien uno letra o un dígito, `false` de lo contrario. Cuando se llama con una cadena vacía, el resultado será siempre `false`.

## Ejemplos

Un ejemplo de `ctype_alnum` (usando la localización predeterminada)

```
<?php
$cadenas = array('AbCd1zyZ9', 'foo!#$bar');
foreach ($cadenas as $caso_prueba) {
    if (ctype_alnum($caso_prueba)) {
        echo "La cadena $caso_prueba consiste completamente de letras o dígitos.\n";
    } else {
        echo "La cadena $caso_prueba no consiste completamente de letras o dígitos.\n";
    }
}
?>

    
```php

El ejemplo anterior mostrará:

    La cadena AbCd1zyZ9 consiste completamente de letras o dígitos.
    La cadena foo!#$bar no consiste completamente de letras o dígitos.

## Véase también

`ctype_alpha`, `ctype_digit`, `setlocale`, `IntlChar::isalnum`
