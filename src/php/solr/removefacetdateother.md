---
title: SolrQuery::removeFacetDateOther
description: Elimina uno de los parámetros facet.date.other
source_url: https://www.php.net/manual/es/solrquery.removefacetdateother.php
source_repo: https://github.com/php/doc-es.git
source_ref: master
source_commit: 954a0d911
source_path: reference/solr/solrquery/removefacetdateother.xml
technology: php
version: master
license: CC-BY-3.0
retrieved_at: '2026-08-02'
section: solr
translation_status: ready
translation_revision: b6e314781
order: 79890
---

SolrQuery::removeFacetDateOther

Elimina uno de los parámetros facet.date.other

## Descripción

```php
public SolrQuery::removeFacetDateOther(string $value, [string $field_override]): SolrQuery
```php

Elimina uno de los parámetros facet.date.other.

## Parámetros

`value`  
El valor

`field_override`  
El nombre del campo.

## Valores devueltos

Devuelve el objeto SolrQuery actual, si se usó el valor de retorno.
