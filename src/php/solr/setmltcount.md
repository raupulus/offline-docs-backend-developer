---
title: SolrQuery::setMltCount
description: Establece el número de documentos similares a devolver en cada resultado
source_url: https://www.php.net/manual/es/solrquery.setmltcount.php
source_repo: https://github.com/php/doc-es.git
source_ref: master
source_commit: 954a0d911
source_path: reference/solr/solrquery/setmltcount.xml
technology: php
version: master
license: CC-BY-3.0
retrieved_at: '2026-08-02'
section: solr
translation_status: ready
translation_revision: b6e314781
order: 80480
---

SolrQuery::setMltCount

Establece el número de documentos similares a devolver en cada resultado

## Descripción

```php
public SolrQuery::setMltCount(int $count): SolrQuery
```php

Establece el número de documentos similares a devolver en cada resultado

## Parámetros

`count`  
El número de documentos similares a devolver en cada resultado

## Valores devueltos

Devuelve el objeto SolrQuery actual, si se usó el valor de retorno.
