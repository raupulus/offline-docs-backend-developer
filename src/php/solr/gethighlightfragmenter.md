---
title: SolrQuery::getHighlightFragmenter
description: Devuelve el generador de trozos de texto para el texto remarcado
source_url: https://www.php.net/manual/es/solrquery.gethighlightfragmenter.php
source_repo: https://github.com/php/doc-es.git
source_ref: master
source_commit: 954a0d911
source_path: reference/solr/solrquery/gethighlightfragmenter.xml
technology: php
version: master
license: CC-BY-3.0
retrieved_at: '2026-08-02'
section: solr
translation_status: ready
translation_revision: '475439775'
order: 79400
---

SolrQuery::getHighlightFragmenter

Devuelve el generador de trozos de texto para el texto remarcado

## Descripción

```php
public SolrQuery::getHighlightFragmenter([string $field_override]): string
```php

Devuelve el generador de trozos de texto para el texto remarcado. Acepta una sobrescritura opcional de campos.

## Parámetros

`field_override`  
El nombre del campo

## Valores devueltos

Devuelve una cadena en caso de éxito y `null` si no se estableció.
