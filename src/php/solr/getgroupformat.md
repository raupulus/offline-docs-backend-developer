---
title: SolrQuery::getGroupFormat
description: Devuelve el valor de group.format
source_url: https://www.php.net/manual/es/solrquery.getgroupformat.php
source_repo: https://github.com/php/doc-es.git
source_ref: master
source_commit: 954a0d911
source_path: reference/solr/solrquery/getgroupformat.xml
technology: php
version: master
license: CC-BY-3.0
retrieved_at: '2026-08-02'
section: solr
translation_status: ready
translation_reviewed: true
translation_revision: 86e7c61ef
order: 79270
---

SolrQuery::getGroupFormat

Devuelve el valor de group.format

## Descripción

```php
public SolrQuery::getGroupFormat(): string
```php

Devuelve el valor de group.format.

## Parámetros

Esta función no contiene ningún parámetro.

## Valores devueltos

## Véase también

SolrQuery::setGroupFormat
