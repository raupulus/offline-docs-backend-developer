---
title: SolrQuery::setTermsLimit
description: Establece el número máximo de términos a devolver
source_url: https://www.php.net/manual/es/solrquery.settermslimit.php
source_repo: https://github.com/php/doc-es.git
source_ref: master
source_commit: 954a0d911
source_path: reference/solr/solrquery/settermslimit.xml
technology: php
version: master
license: CC-BY-3.0
retrieved_at: '2026-08-02'
section: solr
translation_status: ready
translation_revision: b6e314781
order: 80650
---

SolrQuery::setTermsLimit

Establece el número máximo de términos a devolver

## Descripción

```php
public SolrQuery::setTermsLimit(int $limit): SolrQuery
```php

Establece el número máximo de términos a devolver

## Parámetros

`limit`  
El número máximo de términos a devolver. Si el límite es negativo se devolverán todos los términos.

## Valores devueltos

Devuelve el objeto SolrQuery actual, si se usó el valor de retorno.
