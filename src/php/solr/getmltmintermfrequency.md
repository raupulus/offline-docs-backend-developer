---
title: SolrQuery::getMltMinTermFrequency
description: Devuelve la frecuencia bajo la cual los términos serán ignorados en el
  documento fuente
source_url: https://www.php.net/manual/es/solrquery.getmltmintermfrequency.php
source_repo: https://github.com/php/doc-es.git
source_ref: master
source_commit: 954a0d911
source_path: reference/solr/solrquery/getmltmintermfrequency.xml
technology: php
version: master
license: CC-BY-3.0
retrieved_at: '2026-08-02'
section: solr
translation_status: ready
translation_revision: '475439775'
order: 79630
---

SolrQuery::getMltMinTermFrequency

Devuelve la frecuencia bajo la cual los términos serán ignorados en el documento fuente

## Descripción

```php
public SolrQuery::getMltMinTermFrequency(): int
```php

Devuelve la frecuencia bajo la cual los términos serán ignorados en el documento fuente

## Parámetros

Esta función no contiene ningún parámetro.

## Valores devueltos

Devuelve un entero en caso de éxito y `null` si no se estableció.
