---
title: SolrQuery::setFacetPrefix
description: Especifica un prefijo de cadena con el que limitar los términos a los
  que hacer una faceta
source_url: https://www.php.net/manual/es/solrquery.setfacetprefix.php
source_repo: https://github.com/php/doc-es.git
source_ref: master
source_commit: 954a0d911
source_path: reference/solr/solrquery/setfacetprefix.xml
technology: php
version: master
license: CC-BY-3.0
retrieved_at: '2026-08-02'
section: solr
translation_status: ready
translation_revision: b6e314781
order: 80170
---

SolrQuery::setFacetPrefix

Especifica un prefijo de cadena con el que limitar los términos a los que hacer una faceta

## Descripción

```php
public SolrQuery::setFacetPrefix(string $prefix, [string $field_override]): SolrQuery
```php

Especifica un prefijo de cadena con el que limitar los términos a los que hacer una faceta.

## Parámetros

`prefix`  
La cadena de prefijo

`field_override`  
El nombre del campo.

## Valores devueltos

Devuelve el objeto SolrQuery actual, si se usó el valor de retorno.
