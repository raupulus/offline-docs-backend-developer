---
title: SolrQuery::getHighlightFields
description: Devuelve todos los campos que Solr debería generar para remarcación de
  trozos
source_url: https://www.php.net/manual/es/solrquery.gethighlightfields.php
source_repo: https://github.com/php/doc-es.git
source_ref: master
source_commit: 954a0d911
source_path: reference/solr/solrquery/gethighlightfields.xml
technology: php
version: master
license: CC-BY-3.0
retrieved_at: '2026-08-02'
section: solr
translation_status: ready
translation_revision: '475439775'
order: 79380
---

SolrQuery::getHighlightFields

Devuelve todos los campos que Solr debería generar para remarcación de trozos

## Descripción

```php
public SolrQuery::getHighlightFields(): array
```php

Devuelve todos los campos que Solr debería generar para remarcación de trozos

## Parámetros

Esta función no contiene ningún parámetro.

## Valores devueltos

Devuelve una matriz en caso de éxito y `null` si no se estableció.
